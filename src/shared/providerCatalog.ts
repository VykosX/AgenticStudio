...
import { z } from "zod";
import { isToolAliasName, SUPERSEDED_TOOL_NAMES, TOOL_ALIAS_DEFINITIONS } from "../tools/consolidated";
import {
...
import { computeFuzzyScore, normalize } from "./providerUtils";

export function summarizeByPlugin(docs: ToolDocumentation[]): Array<Record<string, unknown>> {
...
export function summarizeToolNamesByPlugin(docs: ToolDocumentation[]): Array<Record<string, unknown>> {
...
export function compactToolRecord(doc: ToolDocumentation): Record<string, unknown> {
  return {
    name: doc.name,
    plugin: doc.plugin,
    pluginLabel: doc.pluginLabel || doc.plugin,
...
export const writeToFileParameter = z.union([z.string(), z.boolean()]).default("");

export function standardToolRecord(doc: ToolDocumentation, includeExamples: boolean): Record<string, unknown> {
  return {
    name: doc.name,
    plugin: doc.plugin,
    pluginLabel: doc.pluginLabel || doc.plugin,
...
export function basicToolRecord(doc: ToolDocumentation): Record<string, unknown> {
  return {
    name: doc.name,
    plugin: doc.plugin,
    pluginLabel: doc.pluginLabel || doc.plugin,
...
export function zodSchemaToJson(value: any): unknown {
  if (!value || typeof value !== "object" || !value._def) return null;
  const def = value._def as Record<string, any>;
  switch (def.typeName) {
    case "ZodString":
      return { type: "string" };
    case "ZodNumber":
    ...
    case "ZodUnion":
      return { anyOf: (def.options || []).map((option: any) => zodSchemaToJson(option)) };
    case "ZodObject": {
      ...
      for (const [key, schema] of Object.entries(shape || {})) {
        ...
        const schemaDef = (schema as any)?._def?.typeName;
...
export function runtimeToolSchema(registeredTools: Tool[], toolName: string): unknown {
  const found = registeredTools.find((registeredTool) => (registeredTool as any)?.name === toolName) as any;
  return found?.parametersSchema ? zodSchemaToJson(found.parametersSchema) : null;
}

function scoreToolMatch(toolName: string, queryTool: string): number {
  const query = normalize(queryTool);
  const name = normalize(toolName);
  if (!query) return 0;
  if (name === query) return 1;
  if (name.includes(query) || query.includes(name)) return 0.92;
  return computeFuzzyScore(query, name);
}

export function findMatchingTool(docs: ToolDocumentation[], queryTool: string): ToolDocumentation | null {
  let best: ToolDocumentation | null = null;
  let bestScore = 0;
  for (const doc of docs) {
...
export function normalizeQueryToolList(value: unknown): string[] {
...
export function findSimilarTools(docs: ToolDocumentation[], target: ToolDocumentation, maxItems = 8): ToolDocumentation[] {
  return docs
...
export function inferToolCategory(name: string, description: string): string {
  const normalizedName = name.toLowerCase();
  const haystack = `${name} ${description}`.toLowerCase();
  if (normalizedName === "consult_secondary_agent" || normalizedName.includes("secondary_agent")) return "delegation";
  if (isToolAliasName(normalizedName)) return "alias";
  if (normalizedName.startsWith("as_tool_") || normalizedName === "as_skill_recommend") return "help";
  if (normalizedName === "as_skill") return "help";
  if (normalizedName === "as_dynamic_tool") return "developer";
  if (normalizedName === "as_memory_controller" || normalizedName === "as_todo_controller") return "memory";
  if (["as_process_controller", "as_service_controller", "as_registry_controller", "as_environ_controller", "as_task_controller"].includes(normalizedName)) return "system";
  if (["as_window_controller", "as_clipboard_controller", "as_input_controller"].includes(normalizedName)) return "desktop";
  if (normalizedName === "as_file_metadata") return "metadata";
  if (["as_file_copy_move", "as_file_create", "as_file_restore", "as_file_watch", "as_file_search", "as_file_compare"].includes(normalizedName)) return "file-management";
  if (normalizedName === "as_file_organize") return "organization";
  if (normalizedName.startsWith("as_clipboard_") || normalizedName.startsWith("as_window_") || normalizedName.startsWith("as_input_") || normalizedName.startsWith("as_screenshot_")) return "desktop";
  if (normalizedName.startsWith("as_port_")) return "system";
  if (normalizedName.startsWith("as_process_") || normalizedName.startsWith("as_service_") || normalizedName.startsWith("as_registry_") || normalizedName.startsWith("as_env_")) return "system";
  if (normalizedName === "as_agent_task" || normalizedName === "as_sleep" || normalizedName === "as_http_wait" || normalizedName.startsWith("as_task_schedule_") || normalizedName.startsWith("as_file_watch_")) return "automation";
  if (normalizedName === "as_date_math") return "date-time";
  if (normalizedName.startsWith("as_metadata_")) return "metadata";
  if (normalizedName.startsWith("as_media_") || normalizedName.startsWith("as_image_") || normalizedName.startsWith("as_mkv_") || normalizedName.startsWith("as_vision_")) return "media";
  if (["as_database_query", "as_tabular_data", "as_structured_data"].includes(normalizedName) || normalizedName.startsWith("as_data_") || normalizedName.endsWith("_query")) return "structured-data";
  if (normalizedName === "as_archive") return "media";
  if (normalizedName === "as_torrent_controller") return "downloads";
  if (normalizedName === "as_multi_website_search" || normalizedName.startsWith("as_web_") || normalizedName.startsWith("as_download_") || normalizedName === "as_http_request") return normalizedName === "as_download_video" ? "downloads" : "web-scraping";
  if (normalizedName === "as_file_rename" || normalizedName.startsWith("as_file_plan_") || normalizedName.startsWith("as_file_preview_") || normalizedName.startsWith("as_file_batch_") || normalizedName.includes("duplicate") || normalizedName.includes("reorganization")) return "organization";
  if (normalizedName.startsWith("as_file_")) return "file-management";
  if (normalizedName === "as_git_controller" || normalizedName.startsWith("as_code_") || normalizedName.startsWith("as_python_") || normalizedName.startsWith("as_cpp_") || normalizedName === "as_project_analyze" || normalizedName === "as_project_verify" || normalizedName === "as_project_bug_scan" || normalizedName === "as_deno_run_script") return "developer";
  if (/\b(help|catalog)\b/.test(haystack)) return "help";
  if (/\b(secondar|subagent|delegat)\b/.test(haystack)) return "delegation";
  if (/\b(math|calculate|arithmetic|trigon|logarithm|factorial|statistic|combinatoric)\b/.test(haystack)) return "math";
  if (/\b(date|time|timezone|weekday|calendar|duration|leap year)\b/.test(haystack)) return "date-time";
  if (/\b(unit|convert|currency|temperature|pressure|torque|storage)\b/.test(haystack)) return "unit-conversion";
  if (/\b(youtube|playlist|yt-dlp|subtitle|thumbnail|video)\b/.test(haystack)) return "downloads";
  if (/\b(firecrawl|scrap|crawl|mhtml|archive page|website|web|url|download)\b/.test(haystack)) return "web-scraping";
  if (/\b(csv|json|yaml|sqlite|postgres|mysql|sql|data)\b/.test(haystack)) return "structured-data";
  if (/\b(metadata|exif)\b/.test(haystack)) return "metadata";
  if (/\b(media|ffmpeg|ffprobe|image|mkv|audio)\b/.test(haystack)) return "media";
  if (/\b(process|service|registry|env|system|port)\b/.test(haystack)) return "system";
  if (/\b(window|clipboard|keyboard|mouse|screenshot|desktop)\b/.test(haystack)) return "desktop";
  if (/\b(schedule|watch|automation|task|workflow|wait|orchestrat|sleep)\b/.test(haystack)) return "automation";
  if (/\b(reorgan|duplicate|rename|organize|cleanup)\b/.test(haystack)) return "organization";
  if (/\b(file|directory|path|trash|undo|diff|patch)\b/.test(haystack)) return "file-management";
  if (/\b(python|cpp|compile|lint|typecheck|project|test|git|branch|commit|clone|verify|bug scan|skill|tool scaffold|tool validate)\b/.test(haystack)) return "developer";
  return "discovered";
}

const aliasToolNames = TOOL_ALIAS_DEFINITIONS.map((entry) => entry.name);

export const TOOL_PROFILE_ORDER: ToolProfile[] = [
  "minimal",
  "file_management",
  "multimedia",
  "web",
...
const minimalProfileToolNames = new Set<string>([
  "as_caveman_status",
  "as_set_tool_profile",
  "as_request_tool",
  "as_tool_catalog",
...
const fileManagementProfileToolNames = new Set<string>([
...
const multimediaProfileToolNames = new Set<string>([
  ...minimalProfileToolNames,
  "as_file_tree",
  "as_file_list",
  "as_file_metadata",
...
const webProfileToolNames = new Set<string>([
  ...minimalProfileToolNames,
  "as_http_request",
  "as_web_search",
  "as_web_image_search",
...
const researchProfileToolNames = new Set<string>([
  ...webProfileToolNames,
  "as_memory_controller",
  "as_todo_controller",
  "as_agent_task",
...
const dataProfileToolNames = new Set<string>([
  ...minimalProfileToolNames,
  "as_file_tree",
  "as_file_list",
  "as_file_read",
...
const desktopProfileToolNames = new Set<string>([
  ...minimalProfileToolNames,
  "as_system_info",
...
const systemAdminProfileToolNames = new Set<string>([
  ...minimalProfileToolNames,
  "as_system_info",
  "as_process_controller",
  "as_port_list",
...
const automationProfileToolNames = new Set<string>([
  ...minimalProfileToolNames,
  "as_system_info",
  "as_file_watch",
  "as_process_controller",
...
const developmentProfileToolNames = new Set<string>([
  ...minimalProfileToolNames,
...
const balancedProfileToolNames = new Set<string>([
...
export function getAllowedToolNamesForProfile(profile: ToolProfile, requestedToolNames: string[] = []): Set<string> | null {
  let allowed: Set<string> | null;
  if (profile === "full") {
    allowed = null;
  } else if (profile === "minimal") {
    allowed = new Set<string>(minimalProfileToolNames);
  } else if (profile === "file_management") {
    allowed = new Set<string>(fileManagementProfileToolNames);
  } else if (profile === "multimedia") {
    allowed = new Set<string>(multimediaProfileToolNames);
  } else if (profile === "web") {
    allowed = new Set<string>(webProfileToolNames);
  } else if (profile === "research") {
    allowed = new Set<string>(researchProfileToolNames);
  } else if (profile === "data") {
    allowed = new Set<string>(dataProfileToolNames);
  } else if (profile === "desktop") {
    allowed = new Set<string>(desktopProfileToolNames);
  } else if (profile === "system_admin") {
    allowed = new Set<string>(systemAdminProfileToolNames);
  } else if (profile === "automation") {
    allowed = new Set<string>(automationProfileToolNames);
  } else if (profile === "development") {
    allowed = new Set<string>(developmentProfileToolNames);
  } else if (profile === "balanced") {
    allowed = new Set<string>(balancedProfileToolNames);
  } else {
    allowed = new Set<string>(minimalProfileToolNames);
  }
...
export function listAgenticStudioToolMinimumProfiles(
  allowIndividualToolRequests = true,
  aliasesEnabled = true,
): Array<{ name: string; minimumProfile: ToolProfile; configNote?: string }> {
  const profilesToScan = TOOL_PROFILE_ORDER.filter((profile) => profile !== "full");
  const minimumByTool = new Map<string, { name: string; minimumProfile: ToolProfile; configNote?: string }>();
  for (const profile of profilesToScan) {
    const allowed = getAllowedToolNamesForProfile(profile) || new Set<string>();
    for (const toolName of allowed) {
      if (toolName === "as_request_tool" && !allowIndividualToolRequests) continue;
      if (isToolAliasName(toolName) && !aliasesEnabled) continue;
...
export function filterToolsByProfile(tools: Tool[], profile: ToolProfile, requestedToolNames: string[] = []): Tool[] {
  const allowed = getAllowedToolNamesForProfile(profile, requestedToolNames);
  if (!allowed) return tools;
  return tools.filter((registeredTool) => allowed.has((registeredTool as any)?.name));
}

export function buildToolDocumentation(
  name: string,
  description: string,
  pluginId: string,
  availability: ToolDocumentation["availability"],
...
function normalizeExtractedDescription(value: string): string {
  return value.replace(/\s+/g, " ").trim();
}

function extractTemplateLiteral(lines: string[], startLine: number, startColumn: number): string {
...
function extractDescriptionFromIdentifier(lines: string[], identifier: string, fromIndex: number): string {
...
function extractDescriptionFromLines(lines: string[], fromIndex: number): string {
...
function extractRegisteredToolsFromSource(text: string, pluginId: string, isLive: boolean): ToolDocumentation[] {
  const docs: ToolDocumentation[] = [];
  const lines = text.split(/\r?\n/);
  for (let i = 0; i < lines.length; i++) {
    if (!/tool\)\s*\(\{|tool\s*\(\{/i.test(lines[i])) continue;
    let name = "";
...
function extractDocumentationFromRuntimeTools(
  registeredTools: Tool[],
  pluginId: string,
  currentProfile: ToolProfile,
  requestedProfile: ToolProfile,
  currentRequestedToolNames: string[] = [],
  allowIndividualToolRequests = true,
): ToolDocumentation[] {
  const docs: ToolDocumentation[] = [];
...
export async function discoverToolDocumentation(
  scope: "own" | "enabled" | "disabled" | "all",
  enabledPluginIds: string[],
  mode: "live_only" | "all_installed" = "live_only",
  registeredOwnTools: Tool[] = [],
  currentProfile: ToolProfile = "minimal",
  requestedProfile: ToolProfile = currentProfile,
  currentRequestedToolNames: string[] = [],
  allowIndividualToolRequests = true,
): Promise<ToolDocumentation[]> {
  const docs = new Map<string, ToolDocumentation>();
  if (scope !== "disabled") {
    for (const doc of extractDocumentationFromRuntimeTools(
      registeredOwnTools,
      "vykosx/agentic-studio",
      currentProfile,
      requestedProfile,
      currentRequestedToolNames,
      allowIndividualToolRequests,
    )) {
      if (!SUPERSEDED_TOOL_NAMES.has(doc.name)) docs.set(`${doc.plugin}:${doc.name}`, doc);
    }
  }
  if (scope === "own") {
    return [...docs.values()]
  ...
  const pluginsRoot = getPluginsRootDirectory();
  const enabledSet = new Set(enabledPluginIds.map((value) => normalize(value)));
  const owners = await fsp.readdir(pluginsRoot, { withFileTypes: true }).catch(() => []);
  for (const ownerEntry of owners) {
    if (!ownerEntry.isDirectory()) continue;
    const ownerPath = path.join(pluginsRoot, ownerEntry.name);
    const plugins = await fsp.readdir(ownerPath, { withFileTypes: true }).catch(() => []);
    for (const pluginEntry of plugins) {
      if (!pluginEntry.isDirectory()) continue;
      const pluginPath = path.join(ownerPath, pluginEntry.name);
      const pluginId = `${ownerEntry.name}/${pluginEntry.name}`;
      const isLive = enabledSet.has(normalize(pluginId)) || normalize(pluginId) === "vykosx/agentic-studio";
      if (normalize(pluginId) === "vykosx/agentic-studio") continue;
      if (scope === "enabled" && !isLive) continue;
      if (scope === "disabled" && isLive) continue;
      if (mode === "live_only" && !isLive) continue;
      ...
      for (const relativeFile of [
        ...
        if (await fileExists(fullPath)) candidateFiles.push(fullPath);
      }
      await collectToolSourceFiles(pluginPath, "src/tools", candidateFiles, 120);
      await collectToolSourceFiles(pluginPath, "SRC/tools", candidateFiles, 120);
      await collectToolSourceFiles(pluginPath, "dist/tools", candidateFiles, 120);
...
export async function discoverPluginInventory(docs: ToolDocumentation[], enabledPluginIds: string[]): Promise<PluginInventoryEntry[]> {
  const enabledSet = new Set(enabledPluginIds.map((value) => normalize(value)));
  ...
  for (const ownerEntry of owners) {
    ...
    for (const pluginEntry of plugins) {
      ...
      const pluginId = `${ownerEntry.name}/${pluginEntry.name}`;
      const pluginPath = path.join(ownerPath, pluginEntry.name);
      const enabled = enabledSet.has(normalize(pluginId));
      let capabilityKind: PluginInventoryEntry["capabilityKind"] = "unknown";
...
export function filterDisabledOnly(docs: ToolDocumentation[], enabledPluginIds: string[]): ToolDocumentation[] {
  const enabledSet = new Set(enabledPluginIds.map((value) => normalize(value)));
  return docs.filter((doc) => doc.plugin !== "vykosx/agentic-studio" && !enabledSet.has(normalize(doc.plugin)));
}

export function recommendToolsForGoal(goal: string, docs: ToolDocumentation[]): ToolDocumentation[] {
  ...
  const picks = new Map<string, { doc: ToolDocumentation; score: number; order: number }>();
  let pickOrder = 0;
  const addPick = (doc: ToolDocumentation, increment: number) => {
  ...
  const addIf = (pattern: RegExp, ...names: string[]) => {
    if (pattern.test(query)) addByName(...names);
  };
  ...
  if (picks.size < 6) {
    const ranked = docsSource
      .map((doc) => {
        ...
        let score = 0;
        score += computeFuzzyScore(normalizedGoal, `${normalize(doc.name)} ${normalize(doc.category)}`) * 5;
        for (const word of normalizedGoal.split(/\s+/).filter((entry) => entry.length >= 3 && !stopwords.has(entry))) {
          if (normalize(doc.name).includes(word)) score += 3;
          else if (haystack.includes(word)) score += 1.2;
        }
        if (query.includes(doc.category)) score += 1;
        return { doc, score };
      })
      .filter((entry) => entry.score > 1.25)
      .sort((left, right) => right.score - left.score || left.doc.name.localeCompare(right.doc.name))
      .slice(0, 8);
    for (const entry of ranked) addPick(entry.doc, entry.score);
  }
  if (picks.size === 0) {
    addByName("as_tool_help", "as_tool_catalog", "as_project_verify", "as_agent_task");
  }
  if (picks.size === 0) {
    for (const doc of docsSource.slice(0, 8)) addPick(doc, 1);
  }
  return [...picks.values()]
