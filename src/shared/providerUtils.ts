...
export const dynamicToolNameSchema = z.string().regex(/^[A-Za-z0-9_-]+$/);

export function normalize(input: string): string {
  return input.toLowerCase().replace(/\s+/g, " ").trim();
}

export function expandEnvironmentPath(input: string): string {
...
function cavemanString(value: string): string {
  return value
...
export function cavemanifyToolResult(value: unknown, key = ""): unknown {
  if (typeof value === "string") {
    if (literalStringKeys.has(key)) return value;
    if (!agentFacingStringKeys.has(key)) return value;
...
export function escapeForPython(value: string): string {
  return value.replace(/\\/g, "\\\\").replace(/"/g, '\\"');
}

export function indentPython(code: string): string {
  return code.split(/\r?\n/).map((line) => `    ${line}`).join("\n");
}
...
export function splitCommandList(value: string | undefined): string[] {
...
export function mergeDefined<T extends Record<string, unknown>>(...objects: T[]): T {
...
export function parseJsonArrayOfStrings(value: string, fieldName: string): string[] {
