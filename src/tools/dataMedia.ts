...
export function registerDataMediaTools(ctx: ToolModuleContext, tools: any[]): void {
  ...
  const resolveTargets = (params: Record<string, unknown>, primaryPath: unknown, primaryPathName = "path", options: Record<string, unknown> = {}) => resolveBatchFileTargets({
    workspaceRoot,
  ...
  const runPowerShellScriptFile = async (scriptContents: string, prefix: string, localTimeoutMs = Math.max(timeoutMs, 120000)) => {
  ...
  const normalizeReadOnlySql = (query: unknown, allowedPrefix: RegExp, toolName: string): string => {
    const trimmed = String(query || "").trim();
    if (!trimmed) throw new Error(`${toolName} query must not be empty.`);
  ...
  const csvRecordsFromRows = (rows: string[][], hasHeader: boolean, limit: number): Array<Record<string, unknown>> => {
    if (hasHeader) {
      return csvRowsToObjects(rows.slice(0, limit + 1), true);
    }
    return rows.slice(0, limit).map((row: string[]) => Object.fromEntries(row.map((value, index) => [`column_${index + 1}`, value])));
  };
  ...
  const compactRowsResult = async (
    ...
    if (detailLevel === "compact" && rows.length > sampledRows.length) {
      base[`${rowKey}ReportPath`] = await maybeWriteToolOutputToFile(
        workspaceRoot,
        createReportPath(toolName, rowKey),
        payload,
      );
  ...
  const buildStructuredReportSummary = async (
    toolName: string,
    detailLevel: string,
    label: string,
    value: unknown,
    ...
    const reportPath = await maybeWriteToolOutputToFile(
      workspaceRoot,
    ...
    const preview = Array.isArray(value)
      ...
      : value && typeof value === "object"
        ? Object.fromEntries(Object.entries(value as Record<string, unknown>).slice(0, detailLevel === "full" ? 20 : 10))
        : value;
    return {
      ...metadata,
      detail: detailLevel,
  ...
  const inferVisionMimeType = (filePath: string) => {
  ...
  const parseEmbeddedImageReferences = (value: unknown): string[] => {
    ...
    while ((match = pattern.exec(text)) !== null) {
      ...
      if (raw) matches.push(raw.replace(/^<|>$/g, ""));
    }
  ...
  const resolveEmbeddedImagePath = (value: string): string => {
    const trimmed = String(value || "").trim();
    if (!trimmed) throw new Error("Embedded image path is empty.");
    ...
    const ensureAllowedAbsolutePath = (absolutePath: string) => {
      const resolved = path.resolve(absolutePath);
      if (allowFullFilesystemAccessMode) return resolved;
  ...
  const saveClipboardImageForVision = async (requestedOutputPath: unknown) => {
  ...
  const collectVisionInputs = async (params: Record<string, unknown>, primaryPath: unknown, primaryPathName: string) => {
    const directTargets = await resolveTargets(params, primaryPath, primaryPathName, { requireFiles: true, allowEmpty: true });
    const collected: Array<Record<string, unknown>> = directTargets.map((target) => ({
    ...
    const markdownInputs = [
      ...
      ...((Array.isArray(params.embed_markdown_list) ? params.embed_markdown_list : []) as unknown[]).map((entry) => String(entry || "")),
  ...
  const callVisionModel = async (imagePaths: Array<{ fullPath: string; label?: string }>, prompt: string, maxTokens: number, requestedModelId = "") => {
    ...
    const configuredModel = String((ctl.getPluginConfig(configSchematics).get("subAgentModelId") as string | undefined) || "").trim();
  ...
  const stripVisionCodeFence = (value: string) => {
    const trimmed = String(value || "").trim();
    const fenced = trimmed.match(/^```(?:json)?\s*([\s\S]*?)\s*```$/i);
    return fenced ? fenced[1].trim() : trimmed;
  };
  const tryParseVisionJson = (value: string): { parsed: any; mode: string } | null => {
    const tryValue = (candidate: string, mode: string) => {
  ...
  const repairVisionMojibake = (value: string): string => {
  ...
  const normalizeVisionText = (value: unknown): string => repairVisionMojibake(typeof value === "string" ? value : String(value ?? "")).trim();
  const dedupeVisionStrings = (values: string[]): string[] => {
    const seen = new Set<string>();
  ...
  const sanitizeVisionSnippet = (value: unknown): string => {
    ...
    if (wrappedQuote) cleaned = normalizeVisionText(wrappedQuote[1]);
    const wrappedCode = cleaned.match(/^`(.+)`$/);
    if (wrappedCode) cleaned = normalizeVisionText(wrappedCode[1]);
    return cleaned;
  };
  const normalizeVisionStringArray = (value: unknown): string[] =>
    Array.isArray(value)
      ? dedupeVisionStrings(value.map((entry) => normalizeVisionText(entry)).filter(Boolean))
      : [];
  const normalizeVisionUiReadings = (value: unknown) =>
    Array.isArray(value)
      ? (() => {
        const seen = new Set<string>();
        return value
          .map((entry) => {
            ...
            const role = normalizeVisionText((entry as Record<string, unknown>).role) || "other";
            const text = normalizeVisionText((entry as Record<string, unknown>).text);
      ...
      : [];
  const extractOcrFallbackFromRawAnswer = (rawAnswer: string, fallbackLabel = "") => {
    const normalizedAnswer = normalizeVisionText(rawAnswer);
    const cleanedLines = normalizedAnswer
      .split(/\r?\n/)
      .map((line) => normalizeVisionText(
        line
          .replace(/^\s*(?:\d+\.\s*|[-*•]\s*)/, "")
          .replace(/\*\*/g, ""),
      ))
    ...
    const uiReadings = normalizeVisionUiReadings(cleanedLines.flatMap((line) => {
      ...
      if (!value) return [];
      if (/(display|result|main large number|large number|value)/i.test(label)) return [{ role: "display", text: value }];
      if (/(expression|operation|history)/i.test(label)) return [{ role: "expression", text: value }];
      if (/(title|top left)/i.test(label)) return [{ role: "title", text: value }];
      if (/(mode|status)/i.test(label)) return [{ role: "status", text: value }];
  ...
  const callStructuredVisionModel = async (
    imagePaths: Array<{ fullPath: string; label?: string }>,
    prompts: string[],
    maxTokens: number,
    requestedModelId: string,
    validator: (parsed: any) => boolean,
  ) => {
    let lastResult: { modelId: string; answer: string } | null = null;
  ...
  const tryReadVisionImageDimensions = async (fullPath: string) => {
    try {
  ...
  const normalizeVisionLocatorPoint = (value: unknown, maxWidth = 0, maxHeight = 0) => {
    if (!value || typeof value !== "object") return null;
    const rawX = Number((value as Record<string, unknown>).x);
    const rawY = Number((value as Record<string, unknown>).y);
    if (!Number.isFinite(rawX) || !Number.isFinite(rawY)) return null;
    const clamp = (input: number, max: number) => {
      const rounded = Math.round(input);
      if (!(max > 0)) return rounded;
  ...
  const normalizeVisionLocatorBounds = (value: unknown, maxWidth = 0, maxHeight = 0) => {
    if (!value || typeof value !== "object") return null;
    const rawX = Number((value as Record<string, unknown>).x);
    const rawY = Number((value as Record<string, unknown>).y);
    const rawWidth = Number((value as Record<string, unknown>).width);
    const rawHeight = Number((value as Record<string, unknown>).height);
...
tools.push(tool({
    ...
    implementation: safeTool("as_vision_target", async (params) => {
      ...
      const inputs = await collectVisionInputs(params as Record<string, unknown>, relPath, "path");
      ...
      const { modelId, rawAnswer, parsed, attempts, parseMode } = await callStructuredVisionModel(
        inputs.map((entry) => ({ fullPath: String(entry.fullPath), label: String(entry.label || "") })),
        prompts,
        effectiveMaxTokens,
        model_id as string,
        (value) => {
          if (!value || typeof value !== "object") return false;
          return Array.isArray((value as Record<string, unknown>).targets)
            || Array.isArray((value as Record<string, unknown>).images);
        },
      );
      const normalizeTargetEntry = (entry: Record<string, unknown>, fallbackLabel: string, maxWidth = 0, maxHeight = 0) => {
      ...
      if (parsed && typeof parsed === "object" && Array.isArray((parsed as Record<string, unknown>).images)) {
        return json({
          ...basePayload,
          results: ((parsed as Record<string, unknown>).images as Record<string, unknown>[])
            .map((entry, index) => {
              const dims = inputImageInfo[index]?.dimensions;
              const targetEntries = Array.isArray(entry.targets) ? entry.targets as Record<string, unknown>[] : [];
              return {
      ...
      if (parsed && typeof parsed === "object") {
        ...
        const targetEntries = Array.isArray((parsed as Record<string, unknown>).targets)
          ? (parsed as Record<string, unknown>).targets as Record<string, unknown>[]
          : [];
        return json({
          ...basePayload,
          summary: normalizeVisionText((parsed as Record<string, unknown>).summary),
          dimensions: dims || undefined,
          targets: requestedTargets.map((targetLabel, targetIndex) => normalizeTargetEntry(targetEntries[targetIndex] || {}, targetLabel, Number(dims?.width) || 0, Number(dims?.height) || 0)),
        });
      }
...
tools.push(tool({
    ...
    implementation: safeTool("as_archive", async (params) => {
      ...
      const inferArchiveFormat = (targetPath: string): "zip" | "tar.gz" | "zstd" | null => {
      ...
      const archiveExtensionForFormat = (format: "zip" | "tar.gz" | "zstd") =>
        format === "zip" ? ".zip" : format === "tar.gz" ? ".tar.gz" : ".zst";
      const stripArchiveExtension = (targetPath: string) => {
        const lower = targetPath.toLowerCase();
        if (lower.endsWith(".tar.gz")) return targetPath.slice(0, -7);
        if (lower.endsWith(".tgz")) return targetPath.slice(0, -4);
        if (lower.endsWith(".zstd")) return targetPath.slice(0, -5);
        if (lower.endsWith(".zst")) return targetPath.slice(0, -4);
        if (lower.endsWith(".zip")) return targetPath.slice(0, -4);
        return targetPath;
      };
      ...
      if (selectedAction === "extract") {
        const targets = await resolveTargets(params, String(archive_path || relPath), "archive_path", { requireFiles: true });
        const destinationRoot = resolveInsideWorkspace(workspaceRoot, destination as string);
      ...
      if (selectedAction === "uncompress") {
        ...
        const batchMode = targets.length > 1 || hasBatchSelection(params);
        const outputRoot = String(output_path || "").trim() ? resolveInsideWorkspace(workspaceRoot, output_path as string) : "";
        const results = [];
        for (const target of targets) {
      ...
      if (selectedAction === "stream") {
        ...
        for (const target of targets) {
          ...
          const script = [
            ...
            `source_path = r"${escapeForPython(target.fullPath)}"`,
            `offset_bytes = ${offset_bytes as number}`,
            `length_bytes = ${length_bytes as number}`,
...
tools.push(tool({
    ...
    implementation: safeTool("as_database_query", async (params) => {
      ...
      if (driver === "sqlite") {
        ...
        JSON.parse(params_json as string);
        ...
        for (const target of targets) {
          ...
          try {
            await fsp.writeFile(paramsPath, params_json as string, "utf8");
            const script = [
              "import json, sqlite3",
            ...
            results.push(await compactRowsResult(
              "as_database_query",
              detailLevel,
              { databasePath: target.relativePath, ...parsed, rowCount: parsed.rowCount ?? rows.length },
              rows,
            ));
          } finally {
      ...
      if (driver === "postgres") {
        ...
        const result = await executeManagedCommand(ctl, command.trim(), { cwd: workspaceRoot, shell, env }, timeout_ms as number, maxOutputBytes);
      ...
      if (driver === "mysql") {
        ...
        if (String(defaults_file || "").trim()) parts.push(`--defaults-extra-file=${quote(resolveInsideWorkspace(workspaceRoot, defaults_file as string))}`);
        if (String(host || "").trim()) parts.push(`-h ${quote(String(host))}`);
        if (typeof port === "number") parts.push(`-P ${Number(port)}`);
        if (String(user || "").trim()) parts.push(`-u ${quote(String(user))}`);
        if (String(database || "").trim()) parts.push(quote(String(database)));
        ...
        const result = await executeManagedCommand(ctl, command, { cwd: workspaceRoot, shell, env: extraEnv }, timeout_ms as number, maxOutputBytes);
    ...
    implementation: safeTool("as_tabular_data", async (params) => {
      ...
      if (action === "read") {
        ...
        for (const target of targets) {
          ...
          const rows = parseCsv(text, delimiter as string);
          const objects = csvRecordsFromRows(rows, has_header as boolean, limit as number);
          results.push(await compactRowsResult(
            "as_tabular_data",
            detailLevel,
            { path: target.relativePath, rowCount: objects.length },
            objects,
          ));
        }
        return json(results.length === 1 ? results[0] : { count: results.length, results });
      ...
      if (action === "write") {
        const parsed = JSON.parse(rows_json as string);
        let rows: string[][];
        if (Array.isArray(parsed) && parsed.every((entry) => Array.isArray(entry))) {
          rows = parsed.map((entry) => (entry as unknown[]).map((value) => String(value ?? "")));
        } else if (Array.isArray(parsed) && parsed.every((entry) => entry && typeof entry === "object" && !Array.isArray(entry))) {
          rows = csvObjectsToRows(parsed as Record<string, unknown>[], include_header as boolean);
        } else {
        ...
        for (const target of targets) {
          ...
          await fsp.writeFile(target.fullPath, `${stringifyCsv(rows, delimiter as string)}\n`, "utf8");
      ...
      if (action === "query") {
        const filters = JSON.parse(filters_json as string) as Record<string, unknown>;
        const results = [];
        for (const target of targets) {
          const text = await fsp.readFile(target.fullPath, "utf8");
          let rows = csvRecordsFromRows(parseCsv(text, delimiter as string), has_header as boolean, 100000);
          for (const [key, rawValue] of Object.entries(filters || {})) {
            rows = rows.filter((row) => String(row[key] ?? "").toLowerCase().includes(String(rawValue ?? "").toLowerCase()));
          }
          if ((sort_by as string).trim()) {
            const key = sort_by as string;
            rows.sort((left, right) => String(left[key] ?? "").localeCompare(String(right[key] ?? "")));
            if (descending) rows.reverse();
          }
          const sliced = rows.slice(0, limit as number);
          results.push(await compactRowsResult(
            "as_tabular_data",
            detailLevel,
            { path: target.relativePath, rowCount: sliced.length, totalMatchedRows: rows.length },
            sliced,
          ));
        }
        return json(results.length === 1 ? results[0] : { count: results.length, results });
...
tools.push(tool({
    ...
    implementation: safeTool("as_file_read", async (params) => {
      ...
      for (const target of targets) {
        ...
        const start = Math.min(offset as number, buffer.length);
        const end = typeof length === "number"
          ? Math.min(buffer.length, start + (length as number))
          : buffer.length;
        ...
        if (mode === "text") {
          ...
          result.text = slice.toString(encoding as BufferEncoding);
        } else {
...
tools.push(tool({
    ...
    implementation: safeTool("as_file_write", async (params) => {
      ...
      let content: Buffer;
      if (mode === "text") {
        if (typeof text !== "string") throw new Error("text is required when mode='text'.");
        content = Buffer.from(text, encoding as BufferEncoding);
      } else {
        if (typeof base64 !== "string") throw new Error("base64 is required when mode='base64'.");
...
tools.push(tool({
    ...
    implementation: safeTool("as_structured_data", async (params) => {
      ...
      if (action === "read") {
        const results = [];
        for (const target of targets) {
          const effectiveFormat = detectStructuredFormat(target.fullPath, format as string);
      ...
      if (action === "write") {
        const parsed = JSON.parse(data_json as string);
        const results = [];
        for (const target of targets) {
          if (await fileExists(target.fullPath) && !overwrite) {
            results.push({ path: target.relativePath, status: "skipped", reason: "Destination exists." });
      ...
      if (action === "merge_patch") {
        const patchObject = JSON.parse(patch_json as string);
        const results = [];
        for (const target of targets) {
          const effectiveFormat = detectStructuredFormat(target.fullPath, format as string);
          const exists = await fileExists(target.fullPath);
          if (!exists && !create_if_missing) throw new Error("Target file does not exist and create_if_missing is false.");
          if (effectiveFormat !== "json") throw new Error("merge_patch currently supports JSON files only.");
...
tools.push(tool({
    ...
    implementation: safeTool("as_vision_ocr", async (params) => {
      ...
      const { modelId, rawAnswer, parsed, attempts, parseMode } = await callStructuredVisionModel(
        ...
        (value) => {
          if (!value || typeof value !== "object") return false;
          if (Array.isArray((value as Record<string, unknown>).images)) return true;
          return typeof (value as Record<string, unknown>).fullText === "string"
            || typeof (value as Record<string, unknown>).text === "string"
            || Array.isArray((value as Record<string, unknown>).lines);
        },
      );
      const normalizeOcrEntry = (entry: Record<string, unknown>, fallbackLabel = "") => {
      ...
      const mergeOcrEntryWithFallback = (entry: Record<string, unknown>, fallbackLabel = "", rawFallbackText = "") => {
        ...
        return {
          ...
          numbers: dedupeVisionStrings([...(normalized.numbers || []), ...((fallback?.numbers || []) as string[])]),
          uiReadings: normalized.uiReadings.length > 0 ? normalized.uiReadings : (fallback?.uiReadings || []),
          displayCandidates: dedupeVisionStrings([...(normalized.displayCandidates || []), ...((fallback?.displayCandidates || []) as string[])]).slice(0, 12),
        };
      };
      const basePayload: Record<string, unknown> = {
        count: inputs.length,
      ...
      if (parsed && typeof parsed === "object") {
        const normalized = mergeOcrEntryWithFallback(parsed as Record<string, unknown>, String(inputs[0]?.label || ""), rawAnswer);
        return json({
          ...basePayload,
...
tools.push(tool({
    ...
    implementation: safeTool("as_vision_recognize", async (params) => {
      ...
      const { modelId, rawAnswer, parsed, attempts, parseMode } = await callStructuredVisionModel(
        ...
        (value) => {
          if (!value || typeof value !== "object") return false;
          return typeof (value as Record<string, unknown>).answer === "string"
      ...
      if (parsed && typeof parsed === "object") {
        return json({
          ...payload,
          answer: normalizeVisionText((parsed as Record<string, unknown>).answer || (parsed as Record<string, unknown>).summary || rawAnswer),
          summary: normalizeVisionText((parsed as Record<string, unknown>).summary || ""),
          entities: normalizeVisionStringArray((parsed as Record<string, unknown>).entities),
          textMentions: normalizeVisionStringArray((parsed as Record<string, unknown>).textMentions),
          confidenceNotes: normalizeVisionStringArray((parsed as Record<string, unknown>).confidenceNotes),
          images: Array.isArray((parsed as Record<string, unknown>).images)
            ? ((parsed as Record<string, unknown>).images as Record<string, unknown>[]).map((entry, index) => ({
...
tools.push(tool({
    ...
    implementation: safeTool("as_vision_focus", async (params) => {
      ...
      const normalizeElements = (value: unknown) =>
        Array.isArray(value)
          ? value
            .map((entry) => {
              if (!entry || typeof entry !== "object") return null;
              ...
              if (!label && !type && !text && !location && !state) return null;
      ...
      if (parsed && typeof parsed === "object") {
        return json({
          ...payload,
          answer: normalizeVisionText((parsed as Record<string, unknown>).answer || rawAnswer),
          elements: normalizeElements((parsed as Record<string, unknown>).elements),
          textSnippets: normalizeVisionStringArray((parsed as Record<string, unknown>).textSnippets),
          numbers: normalizeVisionStringArray((parsed as Record<string, unknown>).numbers),
          confidenceNotes: normalizeVisionStringArray((parsed as Record<string, unknown>).confidenceNotes),
          images: Array.isArray((parsed as Record<string, unknown>).images)
            ? ((parsed as Record<string, unknown>).images as Record<string, unknown>[]).map((entry, index) => ({
              label: normalizeVisionText(entry.label || inputs[index]?.label || ""),
...
tools.push(tool({
    ...
    implementation: safeTool("as_metadata_write", async (params) => {
      ...
      const assignments = Object.entries(tags as Record<string, unknown>).map(([key, value]) => {
...
tools.push(tool({
    ...
    implementation: safeTool("as_media_probe", async (params) => {
      ...
      for (const target of targets) {
        ...
        const parsed = result.stdout ? (() => { try { return JSON.parse(result.stdout); } catch { return null; } })() : null;
        if (!parsed) {
...
tools.push(tool({
    ...
    implementation: safeTool("as_media_transform", async (params) => {
      ...
      const trimmedArgs = (extraArgs as string).trim();
      ...
      for (const target of targets) {
        ...
        if (batchMode) {
          ...
          if (!rawOutput && !rawDirectory && !rawExtension) throw new Error("output_path, output_directory, or output_extension is required for batch media transforms.");
          ...
          if (!String(output_path || "").trim()) throw new Error("output_path is required.");
          outputPath = resolveInsideWorkspace(workspaceRoot, output_path as string);
        }
        await fsp.mkdir(path.dirname(outputPath), { recursive: true });
    ...
    implementation: safeTool("as_image_convert", async (params) => {
      ...
      for (const target of targets) {
        ...
        if (batchMode) {
          ...
          const rawDirectory = String(output_directory || "").trim();
          const rawExtension = String(output_extension || "").trim();
          if (!rawOutput && !rawDirectory && !rawExtension) throw new Error("output_path, output_directory, or output_extension is required for batch image conversion.");
          const outputLooksFile = rawOutput && path.extname(rawOutput);
          const outputDir = rawDirectory
            ? resolveInsideWorkspace(workspaceRoot, rawDirectory)
            : rawOutput
    ...
    implementation: safeTool("as_mkv_info", async (params) => {
      ...
      for (const target of targets) {
        ...
        const summary = {
          ...
          tracks: Array.isArray(parsed.tracks) ? parsed.tracks.map((track: any) => ({
            id: track.id,
          ...
          attachments: Array.isArray(parsed.attachments) ? parsed.attachments.map((attachment: any) => ({
            id: attachment.id,
...
tools.push(tool({
    ...
    implementation: safeTool("as_mkv_extract", async (params) => {
      ...
      const parsedTargets = JSON.parse(targets_json as string);
      const buildTargetPairs = (valueName: string, sourceTarget: Record<string, string>) => {
        if (!Array.isArray(parsedTargets) || parsedTargets.length === 0) {
          throw new Error(`targets_json must be a non-empty JSON array for action='${action as string}'.`);
        }
        return parsedTargets.map((entry, index) => {
          ...
          const rawOutputPath = (entry as Record<string, unknown>).output_path;
          const id = String(rawId ?? "").trim();
          const output = String(rawOutputPath ?? "").trim();
          if (!id) throw new Error(`targets_json entry ${index + 1} is missing id.`);
          if (!output) throw new Error(`targets_json entry ${index + 1} is missing output_path.`);
          if (!/^[0-9]+$/.test(id)) throw new Error(`targets_json entry ${index + 1} has a non-numeric id.`);
      ...
      for (const target of targets) {
        ...
        if (selectedAction === "tracks" || selectedAction === "attachments" || selectedAction === "timestamps_v2") {
          ...
          if (!output) throw new Error("output_path is required for action='chapters'.");
          ...
          const output = String(output_path || "").trim();
          if (!output) throw new Error("output_path is required for action='cuesheet'.");
          const outputLooksFile = path.extname(output);
          ...
          const output = String(output_path || "").trim();
          if (!output) throw new Error("output_path is required for action='tags'.");
          const outputLooksFile = path.extname(output);
