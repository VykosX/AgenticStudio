...
export function registerSystemInfoTool(ctx: ToolModuleContext, tools: any[]): void {
  const {
    tool,
    z,
    safeTool,
  ...
  tools.push(tool({
    ...
    implementation: safeTool("as_system_info", async ({ detailed, fields, field_list, help }) => {
      ...
      const nonEmptyObject = (value: unknown) => value && typeof value === "object" && !Array.isArray(value) && Object.keys(value as Record<string, unknown>).length > 0 ? value : undefined;
      const nonEmptyValue = (value: unknown) => Array.isArray(value) ? nonEmptyArray(value) : nonEmptyObject(value) || (value ?? undefined);
      const parseRequestedFields = (...values: unknown[]): string[] => {
        const parsed: string[] = [];
      ...
      const detailRank = (level: "basic" | "detailed" | "maximum") => level === "maximum" ? 2 : level === "detailed" ? 1 : 0;
      const explicitDetailLevel: "basic" | "detailed" | "maximum" = detailed === "maximum" ? "maximum" : detailed ? "detailed" : "basic";
      const inferRequestedDetailLevel = (fieldNames: string[]): "basic" | "detailed" | "maximum" => {
        let level: "basic" | "detailed" | "maximum" = "basic";
        for (const rawField of fieldNames) {
          const fieldName = rawField.toLowerCase();
          const normalizedFieldName = fieldName.replace(/[_-]+/g, " ");
          if (/librehardwaremonitor|sensortelemetry|sensorreadings|rawsensors/.test(fieldName)) return "maximum";
          if (/^(cpupackages|hardware|connectivity|disks|runtimes|processsummary|usersessions|networkinterfaces)\b/.test(fieldName)) level = "detailed";
          if (/\b(cpu|gpu|drive|disk|temperature|fan|power|clock|pcie|vbios|voltage|load)\b/.test(normalizedFieldName)) level = "detailed";
        }
        return level;
      };
      const detailLevel: "basic" | "detailed" | "maximum" = detailRank(inferRequestedDetailLevel(requestedFields)) > detailRank(explicitDetailLevel)
        ? inferRequestedDetailLevel(requestedFields)
        : explicitDetailLevel;
      const includeDetailed = detailLevel !== "basic";
      const includeMaximum = detailLevel === "maximum";
      const parseTelemetryText = (value: unknown): string | null => {
        const text = String(value ?? "").trim();
        if (!text || /^\[(?:N\/A|Not Supported|Requested functionality has been deprecated)\]$/i.test(text)) return null;
        return text;
      };
      ...
      const parseTelemetryBoolean = (value: unknown): boolean | null => {
        const text = parseTelemetryText(value);
        if (text === null) return null;
        if (/^(enabled|active|on|yes|true)$/i.test(text)) return true;
        if (/^(disabled|inactive|off|no|false)$/i.test(text)) return false;
        return null;
      };
      const normalizeGpuIdentity = (value: unknown): string => String(value ?? "")
        .toLowerCase()
        .replace(/\(.*?\)/g, " ")
        .replace(/\b(nvidia|geforce|amd|radeon|intel)\b/g, " ")
        .replace(/[^a-z0-9]+/g, " ")
        .trim();
      const summarizeHardwareMonitorGpuTelemetry = (entry: Record<string, unknown>) => {
        const sensors = asArray<Record<string, unknown>>(entry.sensors).map((sensor) => ({
        ...
        const pickSensor = (sensorType: string, namePatterns: RegExp[]) =>
          sensors.find((sensor) =>
            sensor.sensorType?.toLowerCase() === sensorType.toLowerCase()
            && namePatterns.some((pattern) => pattern.test(String(sensor.name || ""))));
        const pickValue = (sensorType: string, namePatterns: RegExp[]) => pickSensor(sensorType, namePatterns)?.value ?? null;
        const maxMatchingValue = (sensorType: string, namePatterns: RegExp[]) => {
      ...
      const summarizeHardwareMonitorCpuTelemetry = (entry: Record<string, unknown>) => {
        const sensors = asArray<Record<string, unknown>>(entry.sensors).map((sensor) => ({
          name: typeof sensor.name === "string" ? sensor.name : null,
          sensorType: typeof sensor.sensorType === "string" ? sensor.sensorType : null,
          value: toNumberOrNull(sensor.value),
      ...
      const resolveStructuredPath = (root: unknown, rawPath: string): unknown => {
        ...
        for (const segment of segments) {
          ...
          for (const item of current) {
            ...
            if (indexToken === "") {
              ...
              if (Number.isInteger(numericIndex) && numericIndex >= 0 && numericIndex < value.length) next.push(value[numericIndex]);
            }
          }
          current = next;
      ...
      const buildDriveUsageSummary = (root: Record<string, unknown>) =>
        asArray<Record<string, unknown>>(root.disks).map((disk) => {
      ...
      const buildGpuSummaries = (root: Record<string, unknown>) => asArray<Record<string, unknown>>(resolveStructuredPath(root, "hardware.gpuAdapters"));
      const buildCpuSummaries = (root: Record<string, unknown>) => asArray<Record<string, unknown>>(resolveStructuredPath(root, "cpuPackages"));
      const firstNonNull = (values: unknown[]) => values.find((value) => value !== null && value !== undefined);
      const mapDefined = (items: Record<string, unknown>[], key: string) =>
        items
          .map((item) => item[key])
          .filter((value) => value !== null && value !== undefined);
      const systemInfoAliasResolvers: Record<string, (payload: Record<string, unknown>) => unknown> = {
        ...
        cpu_model: (payload) => firstNonNull(mapDefined(buildCpuSummaries(payload), "name")) ?? firstNonNull(mapDefined(asArray<Record<string, unknown>>(payload.cpus), "model")),
        cpu_models: (payload) => {
          const names = mapDefined(buildCpuSummaries(payload), "name");
          return names.length > 0 ? names : mapDefined(asArray<Record<string, unknown>>(payload.cpus), "model");
        },
      ...
      const collectFieldDescriptors = (value: unknown, prefix = "", seen = new Set<string>(), depth = 0): string[] => {
        if (depth > 6 || value === null || value === undefined) return [...seen];
        if (Array.isArray(value)) {
          if (prefix) seen.add(prefix);
          const sample = value.find((entry) => entry !== null && entry !== undefined);
          if (sample !== undefined) collectFieldDescriptors(sample, `${prefix}[]`, seen, depth + 1);
          return [...seen];
        }
      ...
      const matchDescriptorPattern = (descriptor: string, pattern: string): boolean => {
      ...
      const selectSystemInfoFields = (root: Record<string, unknown>, fieldNames: string[]) => {
      ...
      const parseNvidiaSmiGpuTelemetry = (value: string) =>
        value.split(/\r?\n/).filter(Boolean).map((line) => {
          const [
            ...
            maxGraphicsClockMHz,
            maxMemoryClockMHz,
            pcieLinkGenerationCurrent,
            pcieLinkGenerationMax,
      ...
      const safeJsonCommand = async (command: string, parser = (value: string) => JSON.parse(value)): Promise<unknown> => {
        try {
      ...
      const safeTextCommand = async (command: string): Promise<string | null> => {
        try {
      ...
      const runPowerShellScriptFile = async (scriptContents: string, prefix: string): Promise<string | null> => {
      ...
      const escapePowerShellSingleQuoted = (value: unknown) => `'${String(value ?? "").replace(/'/g, "''")}'`;
      const readLibreHardwareMonitorSnapshot = async (executablePath: string): Promise<unknown> => {
        const tempDirectory = path.join(workspaceRoot, "screenshots", "tool-temp");
        await fsp.mkdir(tempDirectory, { recursive: true });
      ...
      const cpuModelCounts = new Map<string, number>();
      ...
      let disks: unknown = null;
      let physicalDisks: unknown = null;
      let processors: unknown = null;
      let gpuAdapters: unknown = null;
      let gpuTelemetry: unknown = null;
      let gpuSensorTelemetry: unknown = null;
      let cpuSensorTelemetry: unknown = null;
      let libreHardwareMonitorSnapshot: unknown = null;
      let cpuUsage: unknown = null;
      let systemLoad: unknown = null;
      let motherboard: unknown = null;
      let userSessions: unknown = null;
      let processSummary: unknown = null;
      let bluetooth: unknown = null;
      let networkAdapters: unknown = null;
      let networkInterfaces: unknown = null;
      let networkUsage: unknown = null;
      let publicIp: unknown = null;
      let monitors: unknown = null;
      let displayDevices: unknown = null;
      if (process.platform === "win32") {
        ...
        if (path.isAbsolute(libreHardwareMonitorPath) && await fileExists(libreHardwareMonitorPath)) {
          ...
          const libreHardwareItems = asArray<Record<string, unknown>>(libreHardwareMonitorSnapshot);
      ...
      if (includeDetailed) {
        networkInterfaces = Object.fromEntries(
          Object.entries(os.networkInterfaces()).map(([name, values]) => [name, (values || []).map((entry) => ({
            family: entry.family,
      ...
      const processorEntries = asArray<Record<string, unknown>>(processors);
      const inferredProcessorCount = toNumberOrNull((systemLoad as Record<string, unknown> | null)?.NumberOfProcessors) ?? null;
      const cpuPackages = processorEntries.length > 0
        ...
        : inferredProcessorCount && inferredProcessorCount > 0
          ...
          : [];
      const normalizedCpuSensorTelemetry = asArray<Record<string, unknown>>(cpuSensorTelemetry).map((entry) => summarizeHardwareMonitorCpuTelemetry(entry));
      ...
      const normalizedGpuAdapters = asArray<Record<string, unknown>>(gpuAdapters);
      const normalizedGpuTelemetry = asArray<Record<string, unknown>>(gpuTelemetry);
      const normalizedGpuSensorTelemetry = asArray<Record<string, unknown>>(gpuSensorTelemetry).map((entry) => summarizeHardwareMonitorGpuTelemetry(entry));
      const normalizedDisplayDevices = asArray<Record<string, unknown>>(displayDevices);
      const displayDeviceByInstanceId = new Map(
        normalizedDisplayDevices
          .map((entry) => {
            const instanceId = String(entry.InstanceId || "").trim();
            return instanceId ? [instanceId.toUpperCase(), entry] as const : null;
          })
          .filter((entry): entry is readonly [string, Record<string, unknown>] => Boolean(entry)),
      );
      const gpuSensorTelemetryByName = new Map(
        normalizedGpuSensorTelemetry
          .map((entry) => {
      ...
      const gpuSensorTelemetryByIndex = new Map(
        normalizedGpuSensorTelemetry
          .map((entry) => {
            const index = toNumberOrNull(entry.index);
            return index !== null ? [index, entry] as const : null;
          })
      ...
      const normalizedDisks = asArray<Record<string, unknown>>(disks);
      const normalizedPhysicalDisks = asArray<Record<string, unknown>>(physicalDisks);
      const normalizedNetworkAdapters = asArray<Record<string, unknown>>(networkAdapters);
      const normalizedNetworkUsage = asArray<Record<string, unknown>>(networkUsage);
      const normalizedMonitors = asArray<Record<string, unknown>>(monitors).map((entry) => ({
      ...
      const hardware = includeDetailed ? mergeDefined({
        ...
        libreHardwareMonitor: includeMaximum ? nonEmptyObject({
          ...
          hardware: nonEmptyArray(asArray<Record<string, unknown>>(libreHardwareMonitorSnapshot)),
        }) : undefined,
      ...
      if (help) {
        ...
        const validFields = [...new Set([
          ...
          ...collectFieldDescriptors(payload as Record<string, unknown>),
        ])]
      ...
      if (requestedFields.length > 0) {
        return json({
          ...
          detailLevel,
          requestedFields,
          fields: selectSystemInfoFields(payload as Record<string, unknown>, requestedFields),
        });
      }
      return json(payload);
    }),
