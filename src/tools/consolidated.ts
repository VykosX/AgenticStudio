...
export const TOOL_ALIAS_DEFINITIONS: ToolAliasDefinition[] = [
...
export const SUPERSEDED_TOOL_NAMES = new Set<string>([
  "as_file_mkdir",
  "as_file_copy",
  "as_file_move",
  "as_file_trash_list",
...
export function aliasesEnabled(ctl: ToolsProviderController): boolean {
  return (ctl.getPluginConfig(configSchematics).get("enableToolAliases") as boolean | undefined) ?? false;
}

export function findToolAliasDefinition(name: string): ToolAliasDefinition | null {
  const normalized = String(name || "").trim().toLowerCase();
  return TOOL_ALIAS_DEFINITIONS.find((entry) => entry.name.toLowerCase() === normalized) || null;
}

export function isToolAliasName(name: string): boolean {
  return String(name || "").startsWith("ax_");
}

export function finalizeConsolidatedToolSurface(tools: any[], ctl: ToolsProviderController): any[] {
  ...
  for (const registeredTool of tools) {
    ...
    if (SUPERSEDED_TOOL_NAMES.has(name)) continue;
    if (!exposeAliases && isToolAliasName(name)) continue;
    seen.add(name);
...
export function registerConsolidatedTools(ctx: ToolModuleContext, tools: any[]): void {
  const {
    tool,
    z,
    safeTool,
  ...
  const internalToolMap = new Map<string, any>();
  for (const registeredTool of tools) {
    const name = String(registeredTool?.name || "");
    if (name && !internalToolMap.has(name)) internalToolMap.set(name, registeredTool);
  }
  ...
  const aliasDisabled = (definition: ToolAliasDefinition) => json({
    success: false,
    aliasDisabled: true,
  ...
  const unsupported = (toolName: string, supportedPlatforms: string[], message?: string) => json({
    success: false,
    unsupported: true,
    tool: toolName,
    supportedPlatforms,
  ...
  const commandExists = async (commands: string[]): Promise<string | null> => {
    for (const command of commands) {
      ...
      if (!result.error && result.exitCode === 0) return command;
    }
    return null;
  };
  ...
  const parseJsonObjectMaybe = (value: unknown): Record<string, unknown> | null => {
  ...
  const unwrapNestedToolResult = (value: unknown): Record<string, unknown> | null => {
    const parsed = parseJsonObjectMaybe(value);
    if (!parsed) return null;
    const nested = parseJsonObjectMaybe(parsed.result);
    return nested || parsed;
  };
  const withDesktopSkillReminder = (response: any) => {
  ...
  const withInputDispatchReminder = (response: any) => {
    const parsed = parseJsonObjectMaybe(response);
    if (!parsed) return response;
    return json({
      ...parsed,
  ...
  const tryParseFriendlyPressSpec = (value: unknown): { key: string; modifiers: string[] } | null => {
    ...
    for (const part of parts) {
      ...
      else if (part === "win" || part === "leftwin" || part === "rightwin") modifiers.push("win");
      else keys.push(part);
    }
    if (keys.length !== 1) return null;
    return { key: keys[0], modifiers };
  };
  const resolveWindowBoundsForPid = async (pid: number) => {
  ...
  const clampInputDelayMs = (value: unknown, fallback = 100) => Math.max(0, Math.min(10000, Math.trunc(Number(value ?? fallback) || 0)));
  const waitInputDelay = async (delayMs: number) => {
    if (delayMs > 0) await new Promise((resolve) => setTimeout(resolve, delayMs));
  };

  const sh = async (command: string, cwd = workspaceRoot) =>
    executeManagedCommand(ctl, command, { cwd, shell, env }, timeoutMs, maxOutputBytes);

  const mapBaseArgs = (params: Record<string, unknown>) => ({
    query: params.query ?? "",
    limit: params.limit ?? 200,
  });

  const normalizeRegistryControllerValue = (value: unknown, kind: string) => {
  ...
  const resolveWindowTargetForController = async (
    processId: unknown,
    windowId: unknown,
    query: unknown,
    windowTitle: unknown,
    ...
    const scoreWindow = (entry: any, rawSearch: string) => {
      ...
      let value = 0;
      if (title === search) value -= 1000;
      else if (processName === search) value -= 900;
      else if (title.startsWith(search)) value -= 700;
      else if (processName.startsWith(search)) value -= 620;
      else if (titleWords.includes(search)) value -= 520;
      else if (title.includes(search)) value -= 320;
      else if (processName.includes(search)) value -= 240;
      if (documentLikeTitle && processName !== search && !processName.startsWith(search)) value += 180;
      if (/^(applicationframehost|shellexperiencehost|textinputhost)$/.test(processName)) value += 80;
      if (className === "applicationframewindow") value += 20;
      if (processPath.startsWith("c:\\windows\\system32\\")) value += 25;
      return value;
    };
    ...
    if (exactWindowId) {
      ...
      const exactMatch = windows.find((entry: any) =>
        String(entry?.WindowId || "").trim() === exactWindowId
    ...
    const preferredWindow = [...windows].sort((left: any, right: any) => {
  ...
  const resolveWindowPidForController = async (
    processId: unknown,
    windowId: unknown,
    query: unknown,
    windowTitle: unknown,
  ...
  tools.push(tool({
    ...
    implementation: safeTool("as_file_copy_move", async (params) =>
      callTool(params.action === "move" ? "as_file_move" : "as_file_copy", params)),
  }));
    ...
    implementation: safeTool("as_file_create", async (params) => {
      ...
      if (action === "directory") {
        ...
        for (const entry of targets) {
          ...
          if (open_after_create) await callTool("as_file_open", { target: entry.relativePath });
      ...
      const payload = String(content_base64 || "").trim()
        ...
        : Buffer.from(String(content || ""), encoding as BufferEncoding);
      const results = [];
      for (const entry of targets) {
        ...
        if (open_after_create) openResult = await callTool("as_file_open", { target: entry.relativePath });
  ...
  tools.push(tool({
    ...
    implementation: safeTool("as_file_restore", async (params) => {
      const names = [String(params.trash_name || "").trim(), ...(params.file_list as string[] || []).map((entry) => String(entry || "").trim())].filter(Boolean);
      if (names.length === 0) throw new Error("trash_name or file_list is required.");
      if (names.length === 1) {
      ...
      if (!destinationRoot) throw new Error("destination_directory or destination is required when restoring multiple items.");
      const results = [];
  ...
  tools.push(tool({
    ...
    implementation: safeTool("as_file_watch", async ({ action, watch_id, directory, recursive, include_hidden, refresh_snapshot, limit }) => {
      if (action === "list") return await callTool("as_file_watch_list", {});
      if (!String(watch_id || "").trim()) throw new Error("watch_id is required for create, scan, and remove.");
      if (action === "create") return await callTool("as_file_watch_create", { watch_id, directory, recursive, include_hidden, limit });
      if (action === "scan") return await callTool("as_file_watch_scan", { watch_id, refresh_snapshot, limit });
  ...
  tools.push(tool({
    ...
    implementation: safeTool("as_file_organize", async ({ action, source_directory, destination_root, plan_name, plan_id, overwrite, limit }) => {
      if (action === "plan") return await callTool("as_file_plan_reorganization", { source_directory, destination_root, plan_name, limit });
      if (action === "list_plans") return await callTool("as_file_list_reorganization_plans", {});
      if (!String(plan_id || "").trim()) throw new Error("plan_id is required for preview and apply.");
      if (action === "preview") return await callTool("as_file_preview_moves", { plan_id, limit });
  ...
  tools.push(tool({
    ...
    implementation: safeTool("as_file_search", async ({ directory, file_list, folder_list, file_pattern, file_pattern_flags, recursive, include_files, include_directories, include_hidden, extensions, name_patterns, regex_patterns, criteria_json, join, size_min_bytes, size_max_bytes, modified_after, modified_before, created_after, created_before, attributes, file_limit, limit }) => {
      ...
      const criteria = parseJsonArray(criteria_json as string, "criteria_json");
      const addCriterion = (criterion: Record<string, unknown>) => criteria.push(criterion);
      for (const extension of extensions as string[]) addCriterion({ extension });
      for (const pattern of name_patterns as string[]) addCriterion({ name_pattern: pattern });
      for (const pattern of regex_patterns as string[]) addCriterion({ name_regex: pattern });
      if (typeof size_min_bytes === "number" || typeof size_max_bytes === "number") addCriterion({ size_min: size_min_bytes, size_max: size_max_bytes });
      if (modified_after || modified_before) addCriterion({ modified_after, modified_before });
      if (created_after || created_before) addCriterion({ created_after, created_before });
      for (const attribute of attributes as string[]) addCriterion({ attribute });

      const wildcardToRegex = (pattern: string) => new RegExp(`^${pattern.replace(/[.+^${}()|[\]\\]/g, "\\$&").replace(/\*/g, ".*").replace(/\?/g, ".")}$`, "i");
      ...
      const hasAttribute = (entry: Record<string, unknown>, attribute: string) => {
        if (attribute === "file") return entry.type === "file";
        if (attribute === "directory") return entry.type === "directory";
        if (attribute === "symlink") return entry.type === "symlink";
        if (attribute === "hidden") return path.basename(String(entry.path)).startsWith(".");
        if (attribute === "readable") return Boolean(entry.readable);
        if (attribute === "writable") return Boolean(entry.writable);
        if (attribute === "executable") return Boolean(entry.executable);
        return false;
      };
      const matchCriterion = (entry: Record<string, unknown>, criterion: Record<string, unknown>) => {
        const rel = String(entry.path || "");
        const base = path.basename(rel);
        const ext = path.extname(base).toLowerCase().replace(/^\./, "");
        const stat = entry.stat as any;
        if (criterion.type && String(entry.type) !== String(criterion.type)) return false;
        if (criterion.attribute && !hasAttribute(entry, String(criterion.attribute))) return false;
        if (criterion.extension) {
          const wanted = String(criterion.extension).toLowerCase().replace(/^\./, "");
          if (ext !== wanted) return false;
        }
        if (criterion.name && base !== String(criterion.name)) return false;
        if (criterion.name_pattern && !wildcardToRegex(String(criterion.name_pattern)).test(base)) return false;
        if (criterion.name_regex && !(new RegExp(String(criterion.name_regex), criterion.case_sensitive ? "" : "i")).test(base)) return false;
        if (criterion.path_regex && !(new RegExp(String(criterion.path_regex), criterion.case_sensitive ? "" : "i")).test(rel)) return false;
        ...
        if (minSize !== null && Number(entry.sizeBytes || 0) < minSize) return false;
        if (maxSize !== null && Number(entry.sizeBytes || 0) > maxSize) return false;
        ...
        if (mAfter !== null && stat.mtimeMs < mAfter) return false;
        if (mBefore !== null && stat.mtimeMs > mBefore) return false;
        if (cAfter !== null && stat.birthtimeMs < cAfter) return false;
        if (cBefore !== null && stat.birthtimeMs > cBefore) return false;
        return true;
      };
      const results: Array<Record<string, unknown>> = [];
      const considerFullPath = async (fullPath: string) => {
        if (results.length >= Number(limit)) return;
        ...
        try { await fsp.access(fullPath, 4); readable = true; } catch {}
        try { await fsp.access(fullPath, 2); writable = true; } catch {}
        try { await fsp.access(fullPath, 1); executable = true; } catch {}
      ...
      const visit = async (currentDir: string) => {
        ...
        for (const dirent of entries) {
          ...
          if (!include_hidden && dirent.name.startsWith(".")) continue;
          const fullPath = path.join(currentDir, dirent.name);
          const stat = await fsp.lstat(fullPath);
          await considerFullPath(fullPath);
          if (recursive && stat.isDirectory()) await visit(fullPath);
        }
      };
      if (!hasExplicitSearchSelection) await visit(startDir);
      return json({ directory: path.relative(workspaceRoot, startDir) || ".", folderListCount: (folder_list as string[]).length, recursive, join, criteriaCount: criteria.length, count: results.length, results });
    }),
  }));
  ...
  tools.push(tool({
    ...
    implementation: safeTool("as_file_compare", async ({ left_paths_json, right_paths_json, file_list, folder_list, left_file_list, right_file_list, left_folder_list, right_folder_list, file_pattern, file_pattern_flags, left_directory, right_directory, recursive, compare_by, hash_algorithm, modified_tolerance_ms, limit }) => {
      ...
      const gather = async (pathsJson: string, directory: string, fileList: string[], folderList: string[]) => {
        const files: string[] = [];
        ...
        if (String(directory || "").trim()) {
          ...
          if (recursive) files.push(...await collectFiles(dirPath, Number(limit)));
          else {
      ...
      const digest = async (filePath: string) => {
        if (!digestCache.has(filePath)) {
          digestCache.set(filePath, crypto.createHash(hash_algorithm as string).update(await fsp.readFile(filePath)).digest("hex"));
        }
        return digestCache.get(filePath)!;
      };
      const keyFor = async (filePath: string) => {
        const stat = await fsp.stat(filePath);
        const parts = [];
        if ((compare_by as string[]).includes("name")) parts.push(`name=${path.basename(filePath).toLowerCase()}`);
        if ((compare_by as string[]).includes("size")) parts.push(`size=${stat.size}`);
        if ((compare_by as string[]).includes("modified")) {
        ...
        if ((compare_by as string[]).includes("hash")) parts.push(`hash=${await digest(filePath)}`);
        return parts.join("|");
      };
      const leftFiles = await gather(left_paths_json as string, left_directory as string, [...(file_list as string[]), ...(left_file_list as string[])], [...(folder_list as string[]), ...(left_folder_list as string[])]);
      const rightFiles = await gather(right_paths_json as string, right_directory as string, right_file_list as string[], right_folder_list as string[]);
  ...
  tools.push(tool({
    ...
    implementation: safeTool("as_environ_controller", async ({ action, query, name, value }) => {
      if (action === "list") return await callTool("as_env_list", { query });
      if (action === "get") return await callTool("as_env_get", { name });
      if (action === "set") return await callTool("as_env_set", { name, value });
      ...
      if (process.platform === "win32") return await callTool("as_env_refresh", {});
      requireCommandExecution();
      const result = await sh(`${shell} -lc env`);
      if (result.error || result.exitCode !== 0) return buildCommandResponse(`${shell} -lc env`, result);
  ...
  tools.push(tool({
    ...
    implementation: safeTool("as_memory_controller", async ({ action, id, title, content, tags, query, tag, global, limit }) => {
      if (action === "list") return await callTool("as_memory_list", { query, tag, global, limit });
      if (action === "upsert") return await callTool("as_memory_upsert", { id, title, content, tags, global });
      if (action === "delete") return await callTool("as_memory_delete", { id, global });
      ...
      for (const entry of entries) await callTool("as_memory_delete", { id: entry.id, global });
  ...
  tools.push(tool({
    ...
    implementation: safeTool("as_todo_controller", async ({ action, id, text, priority, notes, status, global }) => {
      if (action === "list") return await callTool("as_todo_list", { status, global });
      if (action === "upsert") return await callTool("as_todo_upsert", { id, text, priority, notes, status: status === "all" ? "open" : status, global });
      if (action === "delete") return await callTool("as_todo_delete", { id, global });
      ...
      for (const item of items) await callTool("as_todo_delete", { id: item.id, global });
  ...
  tools.push(tool({
    ...
    implementation: safeTool("as_process_controller", async ({ action, query, include_command_line, pid, force, kill_related, command_line, working_directory, timeout_ms, poll_interval_ms, limit }) => {
      if (action === "list") return await callTool("as_process_list", { query, include_command_line, limit });
      if (action === "details") return await callTool("as_process_details", { pid });
      if (action === "start") return await callTool("as_process_start", { command_line, working_directory });
      if (action === "wait") return await callTool("as_process_wait", { pid, timeout_ms, poll_interval_ms });
      if (action === "kill_tree") {
        if (process.platform === "win32") return await callTool("as_kill_process_tree", { pid, force });
        requireCommandExecution();
  ...
  tools.push(tool({
    ...
    implementation: safeTool("as_service_controller", async ({ action, service_name, query, limit }) => {
      ...
      if (process.platform === "win32") {
        if (action === "list") return await callTool("as_service_list", { query, limit });
        return await callTool("as_service_control", { service_name, action });
      }
      if (process.platform === "linux") {
        const systemctl = await commandExists(["systemctl"]);
        if (!systemctl) return unsupported("as_service_controller", ["win32", "linux", "darwin"], "systemctl was not found.");
        if (action === "list") {
  ...
  tools.push(tool({
    ...
    implementation: safeTool("as_task_controller", async ({ action, task_name, schedule_type, command_line, start_time, start_date, days_interval, query, limit }) => {
      if (action === "list") return await callTool("as_task_schedule_list", { query, limit });
      if (action === "create") return await callTool("as_task_schedule_create", { task_name, schedule_type, command_line, start_time, start_date, days_interval });
      if (action === "delete") return await callTool("as_task_schedule_delete", { task_name });
  ...
  tools.push(tool({
    ...
    implementation: safeTool("as_registry_controller", async ({ action, key_path, value_name, value, value_kind, delete_mode, recursive }) => {
      if (process.platform !== "win32") return unsupported("as_registry_controller", ["win32"], "Registry editing is only meaningful on Windows.");
      if (action === "list") return await callTool("as_registry_list", { key_path });
      if (action === "get") return await callTool("as_registry_get_value", { key_path, value_name });
      if (action === "set") {
  ...
  tools.push(tool({
    ...
    implementation: safeTool("as_clipboard_controller", async ({ action, mode, text, file_paths_json, image_path, save_image_to_path }) => {
      ...
      if (mode !== "text") return unsupported("as_clipboard_controller", ["win32", "linux", "darwin"], "Non-text clipboard modes need Windows clipboard APIs in this plugin.");
      if (process.platform === "darwin") {
      ...
      if (action === "read") {
        if (!reader) return unsupported("as_clipboard_controller", ["win32", "linux", "darwin"], "Install wl-clipboard, xclip, or xsel for Linux clipboard reads.");
      ...
      if (!writer) return unsupported("as_clipboard_controller", ["win32", "linux", "darwin"], "Install wl-clipboard, xclip, or xsel for Linux clipboard writes.");
  ...
  tools.push(tool({
    ...
    implementation: safeTool("as_window_controller", async ({ action, query, process_id, window_id, window_title, control_name, control_window_id, automation_id, control_type, message, w_param, l_param, use_post_message, include_offscreen, max_depth, match_index, x, y, width, height, limit }) => {
      requireCommandExecution();
      if (process.platform === "win32") {
        if (action === "list") return withDesktopSkillReminder(await callTool("as_window_list", { query, limit }));
        ...
        if (action === "foreground" && !hasTargetSelector) return withDesktopSkillReminder(await callTool("as_window_get_foreground", {}));
        ...
        if (action === "close") return withDesktopSkillReminder(await callTool("as_window_close", { pid }));
        if (action === "minimize") return withDesktopSkillReminder(await callTool("as_window_minimize", { pid, action: "minimize" }));
        if (action === "maximize") return withDesktopSkillReminder(await callTool("as_window_maximize", { pid }));
        if (action === "get_bounds") return withDesktopSkillReminder(await callTool("as_window_get_bounds", { pid }));
        if (action === "controls") return withDesktopSkillReminder(await callTool("as_window_list_controls", { pid, window_id: String(target?.windowId || window_id || "").trim(), name: control_name, automation_id, control_type, include_offscreen, max_depth, limit }));
        if (action === "invoke_control") return withDesktopSkillReminder(await callTool("as_window_invoke_control", { pid, window_id: String(target?.windowId || window_id || "").trim(), name: control_name, automation_id, control_type, include_offscreen, max_depth, match_index }));
        if (action === "send_message") {
          const rootWindowId = String(target?.windowId || window_id || "").trim();
          let exactControlWindowId = String(control_window_id || "").trim();
          let selectedControl: any = null;
          ...
          if (!exactControlWindowId) {
            ...
            const win32Controls = candidateControls.filter((entry: any) => String(entry?.source || "").toLowerCase() === "win32" && entry?.messageDispatchable !== false);
      ...
      if (!xdotool) return unsupported("as_window_controller", ["win32", "linux", "darwin"], "Install xdotool or wmctrl for Linux window control.");
      ...
      if (action === "foreground" && !process_id && !String(window_title || query || "").trim()) return buildCommandResponse("xdotool getactivewindow getwindowname", await sh("xdotool getactivewindow getwindowname"));
      if (action === "foreground" || action === "focus") return buildCommandResponse(`xdotool windowactivate ${selector}`, await sh(`xdotool windowactivate ${selector}`));
      if (action === "close") return buildCommandResponse(`xdotool windowclose ${selector}`, await sh(`xdotool windowclose ${selector}`));
      if (action === "minimize") return buildCommandResponse(`xdotool windowminimize ${selector}`, await sh(`xdotool windowminimize ${selector}`));
      if (action === "maximize") return buildCommandResponse(`wmctrl -ir ${selector} -b add,maximized_vert,maximized_horz`, await sh(`wmctrl -ir ${selector} -b add,maximized_vert,maximized_horz`));
      if (action === "get_bounds") return buildCommandResponse(`xdotool getwindowgeometry --shell ${selector}`, await sh(`xdotool getwindowgeometry --shell ${selector}`));
  ...
  tools.push(tool({
    ...
    implementation: safeTool("as_input_controller", async ({ action, text, combo, key, script, script_args_json, script_timeout_ms, key_action, modifiers_json, repeat_count, process_id, window_id, window_title, query, lock_user_input, steps_json, mouse_action, coordinate_space, x, y, move_steps, move_duration_ms, from_x, from_y, to_x, to_y, drag_button, hold_before_drag_ms, hold_after_drag_ms, amount, scroll_axis, send_enter, delay_ms }) => {
      if (action === "help") return inputHelpPayload();
      requireCommandExecution();
      ...
      let targetWindowBounds: Record<string, unknown> | null = null;
      ...
      if (process.platform === "win32") {
        if (action === "type") return withInputDispatchReminder(await callTool("as_input_keyboard", { text, send_enter, delay_ms: delay, ...targetArgs }));
        ...
        if (action === "key_event") return withInputDispatchReminder(await callTool("as_input_key_event", { key: key || combo || text, key_action, modifiers_json, repeat_count, delay_ms: delay, ...targetArgs }));
        if (action === "autohotkey_script") return withInputDispatchReminder(await callTool("as_input_autohotkey", { script: script || text, args_json: script_args_json, timeout_ms: script_timeout_ms, pid: targetArgs.pid, window_id: targetArgs.window_id }));
        if (action === "sequence") return withInputDispatchReminder(await callTool("as_input_keyboard_sequence", { steps_json, delay_ms: delay, ...targetArgs }));
        if (action === "mouse") {
        ...
        if (action === "scroll") return withInputDispatchReminder(await callTool("as_input_mouse_scroll", { amount, axis: scroll_axis, delay_ms: delay, ...targetArgs }));
        if (action === "drag") {
      ...
      if (process.platform === "darwin") {
        ...
        if (!cliclick) return unsupported("as_input_controller", ["win32", "linux", "darwin"], "Install cliclick for macOS mouse and shortcut automation beyond text typing.");
        if (action === "mouse") {
      ...
      const xdotool = await commandExists(["xdotool"]);
      if (!xdotool) return unsupported("as_input_controller", ["win32", "linux", "darwin"], "Install xdotool for Linux input automation.");
      if (action === "type" || action === "paste_text") {
      ...
      if (action === "hotkey" || action === "press") { const command = `xdotool key --clearmodifiers ${quote(String(combo || text).replace(/\+/g, "+"))}`; const response = buildCommandResponse(command, await sh(command)); await waitInputDelay(delay); return response; }
      if (action === "sequence") {
        const steps = parseJsonArray(steps_json as string, "steps_json");
        const commands = steps.map((step) => {
          if (step.text) return `xdotool type --clearmodifiers ${quote(String(step.text))}`;
          if (step.combo) return `xdotool key --clearmodifiers ${quote(String(step.combo))}`;
          if (step.delay_ms) return `sleep ${Math.max(0, Number(step.delay_ms) / 1000)}`;
          if (delay > 0) return `sleep ${delay / 1000}`;
  ...
  for (const definition of TOOL_ALIAS_DEFINITIONS) {
    tools.push(tool({
      ...
      implementation: safeTool(definition.name, async (params) => {
        if (!aliasesEnabled(ctl)) return aliasDisabled(definition);
