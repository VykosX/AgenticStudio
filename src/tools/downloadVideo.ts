...
export function registerDownloadVideoTool(ctx: ToolModuleContext, tools: any[]): void {
  const {
    tool,
    z,
    safeTool,
  ...
  const collectRecentFiles = async (directory: string, startedAtMs: number): Promise<string[]> => {
  ...
  tools.push(tool({
    ...
    implementation: safeTool("as_download_video", async ({
      ...
      const destinationDirectory = resolveInsideWorkspace(workspaceRoot, output_directory as string);
      await fsp.mkdir(destinationDirectory, { recursive: true });
      const startedAtMs = Date.now();

      const parts: string[] = [quote(ytDlp), "--newline", `-P ${quote(destinationDirectory)}`, `-o ${quote(output_template as string)}`];
      if (denoExecutable) {
        parts.push(`--js-runtimes ${quote(`deno:${denoExecutable}`)}`);
      }

      if (playlist_mode === "single") parts.push("--no-playlist");
      if (playlist_mode === "playlist") parts.push("--yes-playlist");

      if (mode === "audio") {
        parts.push("-x");
        if (extract_audio_format !== "best") {
          parts.push(`--audio-format ${quote(extract_audio_format as string)}`);
        }
      }

      const formatSelector = (() => {
        if (String(format_selector || "").trim()) return String(format_selector).trim();
        const codecMap: Record<string, string> = { av1: "av01", vp9: "vp9", h264: "avc1" };
        ...
        if (quality === "1080p") return `bestvideo[height<=1080]${codecFilter}+bestaudio/best[height<=1080]`;
        if (quality === "720p") return `bestvideo[height<=720]${codecFilter}+bestaudio/best[height<=720]`;
        if (quality === "480p") return `bestvideo[height<=480]${codecFilter}+bestaudio/best[height<=480]`;
        if (quality === "worst") return mode === "video_and_audio" ? "worstvideo+worstaudio/worst" : `worst${codecFilter}`;
        return `bestvideo${codecFilter}+bestaudio/best`;
      })();
      if (formatSelector) parts.push(`-f ${quote(formatSelector)}`);

      if (subtitle_mode === "manual" || subtitle_mode === "all") parts.push("--write-subs");
      if (subtitle_mode === "auto" || subtitle_mode === "all") parts.push("--write-auto-subs");
      ...
      if (embed_subtitles) parts.push("--embed-subs");

      if (String(audio_language || "").trim()) {
        parts.push(`--extractor-args ${quote(`youtube:lang=${String(audio_language).trim()}`)}`);
      }
      if (write_thumbnail) parts.push("--write-thumbnail");
      if (write_description) parts.push("--write-description");
      if (write_info_json) parts.push("--write-info-json");
      if (write_comments) parts.push("--write-comments");
      if (embed_metadata) parts.push("--embed-metadata");
      if (embed_thumbnail) parts.push("--embed-thumbnail");
      if (filename_restrict) parts.push("--restrict-filenames");
      if (String(date_after || "").trim()) parts.push(`--dateafter ${quote(String(date_after).trim())}`);
      if (merge_output_format !== "auto" && mode !== "audio") parts.push(`--merge-output-format ${quote(String(merge_output_format))}`);
      if (String(extra_args || "").trim()) {
        assertNoShellControlOperators(extra_args, "extra_args");
        parts.push(String(extra_args).trim());
      }

      parts.push(quote(url as string));
      const command = parts.join(" ");
      const result = await executeManagedCommand(
        ctl,
        command,
        { cwd: destinationDirectory, shell, env },
        timeout_ms as number,
        Math.max(maxOutputBytes, 500000),
      );
