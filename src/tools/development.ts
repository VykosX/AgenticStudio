...
export function registerDevelopmentTools(ctx: ToolModuleContext, tools: any[]): void {
  ...
  const compactCollectionResult = async (
    toolName: string,
    detailLevel: string,
    key: string,
    items: unknown[],
    metadata: Record<string, unknown>,
    mapper: (entry: unknown) => unknown = (entry) => entry,
  ) => {
    if (detailLevel === "max") return { ...metadata, detail: detailLevel, [key]: items };
    ...
    if (normalized.length > sample.length) {
      payload[`${key}ReportPath`] = await maybeWriteToolOutputToFile(
        workspaceRoot,
  ...
  const compactValueResult = async (
    toolName: string,
    detailLevel: string,
    label: string,
    value: unknown,
    ...
    if (detailLevel === "max") return { ...metadata, detail: detailLevel, [label]: value };
    if (value === undefined || value === null || value === "") return { ...metadata, detail: detailLevel };
    if (typeof value === "string") {
      ...
      if (value.length > preview.length) {
        payload[`${label}ReportPath`] = await maybeWriteToolOutputToFile(
          workspaceRoot,
          createReportPath(toolName, label, "txt"),
          { [label]: value },
        );
        payload.reportReadGuidance = REPORT_READ_GUIDANCE;
      }
    ...
    if (typeof value === "object") {
      ...
      const preview = Object.fromEntries(Object.entries(value as Record<string, unknown>).slice(0, detailLevel === "full" ? 25 : 10));
      const payload: Record<string, unknown> = {
        ...metadata,
        detail: detailLevel,
        [detailLevel === "full" ? label : `${label}Preview`]: detailLevel === "full" ? preview : preview,
        [`${label}Keys`]: Object.keys(value as Record<string, unknown>),
  ...
  const resolveGitDirectory = (directory: string): string => {
    return resolveInsideWorkspace(workspaceRoot, directory || ".");
  };

  const resolvePathspec = (cwd: string, pathspec: string): string => {
  ...
  const detectOutlineLanguage = (filePath: string, requested: string): string => {
  ...
  const buildCodeOutline = (text: string, language: string, limit: number) => {
    const lines = text.split(/\r?\n/);
    const symbols: Array<Record<string, unknown>> = [];
    const add = (lineNumber: number, kind: string, name: string, signature: string, level?: number) => {
    ...
    lines.forEach((line, index) => {
      ...
      if (language === "markdown") {
        ...
        if (match) add(lineNumber, "heading", match[2], line, match[1].length);
        return;
      }
      if (language === "json") {
        const match = line.match(/^\s*"([^"]+)"\s*:/);
        if (match) add(lineNumber, "property", match[1], line);
        return;
      }
      if (language === "python") {
        const match = line.match(/^\s*(async\s+def|def|class)\s+([A-Za-z_][A-Za-z0-9_]*)\b(.*)$/);
        if (match) add(lineNumber, match[1].includes("class") ? "class" : "function", match[2], line);
        return;
      }
      if (language === "typescript" || language === "javascript") {
        ...
        if (arrowMatch) add(lineNumber, "function", arrowMatch[1], line);
      }
    });
    return symbols;
  };

  const parseGitPaths = (value: unknown): string[] => {
    return parseJsonArrayOfStrings(String(value || "[]"), "paths_json").map((entry) => entry.trim()).filter(Boolean);
  };

  const resolveGitPathspecs = (cwd: string, paths: string[]): string[] => {
    return paths.map((entry) => resolvePathspec(cwd, entry));
  };

  const buildGitPathspecSuffix = (cwd: string, paths: string[]): string => {
    const relPaths = resolveGitPathspecs(cwd, paths);
    if (relPaths.length === 0) return "";
    return ` -- ${relPaths.map((entry) => quote(entry)).join(" ")}`;
  };

  const runStructuredCommand = async (label: string, command: string, cwd: string, commandTimeout = timeoutMs) => {
  ...
  const detectProjectVerificationCommands = async (cwd: string, options: {
    includeLint: boolean;
    ...
    const commands: Array<{ label: string; command: string; timeoutMs?: number }> = [];
    const packageJsonPath = path.join(cwd, "package.json");
    if (await fileExists(packageJsonPath)) {
      const pkg = JSON.parse(await fsp.readFile(packageJsonPath, "utf8"));
      const scripts = pkg?.scripts && typeof pkg.scripts === "object" ? pkg.scripts : {};
      if (options.includeLint && typeof scripts.lint === "string") commands.push({ label: "lint", command: "npm run lint", timeoutMs: Math.max(timeoutMs, 120000) });
      if (options.includeTypecheck) {
        if (typeof scripts.typecheck === "string") commands.push({ label: "typecheck", command: "npm run typecheck", timeoutMs: Math.max(timeoutMs, 120000) });
        else if (typeof scripts["check-types"] === "string") commands.push({ label: "typecheck", command: "npm run check-types", timeoutMs: Math.max(timeoutMs, 120000) });
      }
      if (options.includeTests && typeof scripts.test === "string") commands.push({ label: "test", command: "npm test", timeoutMs: Math.max(timeoutMs, 180000) });
      if (options.includeBuild && typeof scripts.build === "string") commands.push({ label: "build", command: "npm run build", timeoutMs: Math.max(timeoutMs, 180000) });
    }
  ...
  const collectBugMarkers = async (cwd: string, limit: number) => {
    ...
    for (const filePath of files) {
      ...
      for (const pattern of patterns) {
        ...
        if (!match || findings.length >= limit) continue;
...
tools.push(tool({
    ...
    implementation: safeTool("as_project_analyze", async ({ directory }) => {
      ...
      const cwd = resolveInsideWorkspace(workspaceRoot, directory as string);
      const packageJsonPath = path.join(cwd, "package.json");
      let command = "";
      if (await fileExists(packageJsonPath)) {
        const pkg = JSON.parse(await fsp.readFile(packageJsonPath, "utf8"));
      ...
      if (!command) throw new Error("No supported analyzer detected.");
...
tools.push(tool({
    ...
    implementation: safeTool("as_project_verify", async ({ directory, include_lint, include_typecheck, include_tests, include_build, stop_on_failure }) => {
      ...
      for (const item of commands) {
        ...
        results.push(result);
        if (!result.success && stop_on_failure) break;
      }
      return json({
...
tools.push(tool({
    ...
    implementation: safeTool("as_project_bug_scan", async ({ directory, marker_limit, run_verification, detail }) => {
      ...
      const findings = await collectBugMarkers(cwd, marker_limit as number);
      ...
      return json(await compactCollectionResult(
        "as_project_bug_scan",
        detailLevel,
        "findings",
        findings,
...
tools.push(tool({
    ...
    implementation: safeTool("as_python_format", async ({ paths, formatter, check }) => {
      requireCommandExecution();
      const target = quote(resolveInsideWorkspace(workspaceRoot, paths as string));
      const candidates = formatter === "auto" ? ["ruff", "black", "autopep8"] : [formatter as string];
      let command = "";
      ...
      if (!command) throw new Error("No supported formatter found.");
...
tools.push(tool({
    ...
    implementation: safeTool("as_python_lint", async ({ paths, linter }) => {
      requireCommandExecution();
      const target = quote(resolveInsideWorkspace(workspaceRoot, paths as string));
      const candidates = linter === "auto" ? ["ruff", "flake8", "pylint"] : [linter as string];
      let command = "";
      let fallbackMode = "";
      for (const candidate of candidates) {
        if (candidate === "ruff" && await pythonModuleAvailable("ruff", workspaceRoot, pythonExecutable, shell, env, timeoutMs, maxOutputBytes)) {
...
tools.push(tool({
    ...
    implementation: safeTool("as_python_typecheck", async ({ paths, tool_name }) => {
      requireCommandExecution();
      const target = quote(resolveInsideWorkspace(workspaceRoot, paths as string));
      const candidates = tool_name === "auto" ? ["mypy", "pyright"] : [tool_name as string];
      let command = "";
      let fallbackMode = "";
      for (const candidate of candidates) {
...
tools.push(tool({
    ...
    implementation: safeTool("as_python_run_tests", async ({ paths, runner }) => {
      requireCommandExecution();
      const target = resolveInsideWorkspace(workspaceRoot, paths as string);
      let command = "";
      ...
      if (!command) throw new Error("No supported Python test runner found.");
...
tools.push(tool({
    ...
    implementation: safeTool("as_cpp_get_info", async () => {
      requireCommandExecution();
      const compilerPreference = (ctl.getPluginConfig(configSchematics).get("compilerPreference") as string | undefined) || "auto";
      ...
      for (const compiler of compilerOrder) {
        ...
        const versionResult = await executeManagedCommand(
          ctl,
          resolved.versionCommand || resolved.command,
          { cwd: workspaceRoot, shell: resolved.shellOverride || shell, env },
          timeoutMs,
          maxOutputBytes,
        );
        compilers.push({
...
tools.push(tool({
    ...
    implementation: safeTool("as_cpp_compile_and_run", async (params) => {
      ...
      const requestedOutputPath = resolveInsideWorkspace(workspaceRoot, output as string);
      ...
      const selected = requestedCompilers
        ...
        .find((candidate): candidate is ResolvedCompiler => !!candidate);
      if (!selected) throw new Error("No supported C++ compiler found. On Windows, agentic-studio also checks Visual Studio Build Tools via vcvars64.bat.");
      ...
      for (const target of targets) {
        ...
        const compileCommand = selected.name === "cl"
          ? `${selected.command} /nologo /EHsc /std:${std as string} ${quote(target.fullPath)} /Fe:${quote(outputPath)}`
          : `${selected.command} -std=${std as string} ${quote(target.fullPath)} -o ${quote(outputPath)}`;
        ...
        const runCommandText = `${quote(outputPath)} ${run_args as string}`.trim();
...
tools.push(tool({
    ...
    implementation: safeTool("as_git_controller", async (args) => {
      ...
      const ensureSafeToken = (value: unknown, fieldName: string, allowEmpty = true) => {
        const text = String(value || "").trim();
        if (!text && allowEmpty) return text;
        if (!safeRevisionPattern.test(text)) throw new Error(`${fieldName} contains unsupported git revision characters.`);
        return text;
      };
      const cwd = action === "clone" || action === "auth_status" || action === "login"
        ? workspaceRoot
        : resolveGitDirectory(args.directory as string);
      const runGit = (command: string, commandTimeout = timeoutMs) =>
        buildManagedCommandResponse(ctl, command, { cwd, shell, env }, commandTimeout, maxOutputBytes);

      if (action === "status") {
        return runGit(args.porcelain ? "git status --porcelain=v1 --branch" : "git status --short --branch");
      }
      if (action === "diff") {
        const relPathspec = resolvePathspec(cwd, args.pathspec as string);
        const parts = ["git diff --no-ext-diff"];
        if (args.staged) parts.push("--cached");
        parts.push(args.stat ? "--stat" : `-U${Number(args.context_lines)}`);
        if (relPathspec) parts.push("--", quote(relPathspec));
      ...
      if (action === "show") {
        ...
        if (args.patch) parts.push("--patch");
        else if (!args.stat) parts.push("--no-patch");
        parts.push(quote(ensureSafeToken(args.revision, "revision", false)));
        return runGit(parts.join(" "));
      }
      if (action === "init") {
        const parts = ["git init"];
        if (args.bare) parts.push("--bare");
        const branch = ensureSafeToken(args.branch, "branch");
        if (branch) parts.push(`--initial-branch ${quote(branch)}`);
        return runGit(parts.join(" "));
      }
      if (action === "clone") {
        const destinationPath = resolveInsideWorkspace(workspaceRoot, args.destination as string);
        if (!String(args.repo_url || "").trim()) throw new Error("repo_url is required for clone.");
        if (!String(args.destination || "").trim()) throw new Error("destination is required for clone.");
        if (await fileExists(destinationPath)) throw new Error("Destination already exists.");
        await fsp.mkdir(path.dirname(destinationPath), { recursive: true });
        const parts = ["git clone"];
        const branch = ensureSafeToken(args.branch, "branch");
        if (branch) parts.push(`--branch ${quote(branch)}`);
        if (Number(args.depth) > 0) parts.push(`--depth ${Number(args.depth)}`);
        if (args.recurse_submodules) parts.push("--recurse-submodules");
      ...
      if (action === "add") {
        ...
        if (!args.all && paths.length === 0) throw new Error("Provide paths_json or set all=true.");
        const parts = ["git add"];
        if (args.all) parts.push("--all");
        if (args.update) parts.push("--update");
        if (args.force) parts.push("--force");
        return runGit(`${parts.join(" ")}${buildGitPathspecSuffix(cwd, paths)}`);
      }
      if (action === "commit") {
        if (!String(args.message || "").trim()) throw new Error("message is required for commit.");
        const parts = ["git commit", `-m ${quote(args.message as string)}`];
        if (args.all) parts.push("--all");
        if (args.allow_empty) parts.push("--allow-empty");
        if (args.no_verify) parts.push("--no-verify");
      ...
      if (action === "branch_create") {
        ...
        if (startPoint) parts.push(quote(startPoint));
      ...
      if (action === "restore") {
        const paths = parseGitPaths(args.paths_json);
        if (paths.length === 0) throw new Error("paths_json must contain at least one path.");
        const source = ensureSafeToken(args.source, "source");
        const parts = ["git restore"];
        if (args.staged) parts.push("--staged");
        if (args.worktree) parts.push("--worktree");
        if (source) parts.push(`--source ${quote(source)}`);
        return runGit(`${parts.join(" ")}${buildGitPathspecSuffix(cwd, paths)}`);
      }
      if (action === "fetch") {
        ...
        if (!args.all && remote) parts.push(quote(remote));
      ...
      if (action === "pull") {
        ...
        if (args.rebase) parts.push("--rebase");
        if (args.ff_only) parts.push("--ff-only");
        if (remote) parts.push(quote(remote));
        if (branch) parts.push(quote(branch));
        return runGit(parts.join(" "), Math.max(timeoutMs, 180000));
      }
      if (action === "push") {
        ...
        if (args.set_upstream) parts.push("--set-upstream");
        if (args.force_with_lease) parts.push("--force-with-lease");
        if (args.tags) parts.push("--tags");
      ...
      if (action === "reset") {
        const paths = parseGitPaths(args.paths_json);
        if (paths.length > 0 && args.mode !== "mixed") throw new Error("Path-scoped reset only supports mode='mixed'.");
        if (args.mode === "hard" && !args.confirm_destructive) throw new Error("Hard reset requires confirm_destructive=true.");
        const parts = ["git reset"];
        if (paths.length === 0) parts.push(`--${args.mode as string}`);
        parts.push(quote(ensureSafeToken(args.revision, "revision", false)));
        return runGit(`${parts.join(" ")}${buildGitPathspecSuffix(cwd, paths)}`);
      }
      if (action === "stash_push") {
        ...
        if (args.include_untracked) parts.push("--include-untracked");
        if (args.keep_index) parts.push("--keep-index");
        if (String(args.message || "").trim()) parts.push(`--message ${quote(args.message as string)}`);
      ...
      if (action === "stash_apply") {
        ...
        if (args.restore_index) parts.push("--index");
        parts.push(quote(ensureSafeToken(args.stash_ref, "stash_ref", false)));
        return runGit(parts.join(" "), Math.max(timeoutMs, 180000));
      }
      if (action === "clean") {
        if (!args.dry_run && !args.confirm_destructive) throw new Error("Non-dry-run clean requires confirm_destructive=true.");
        const paths = parseGitPaths(args.paths_json);
        const parts = ["git clean", args.dry_run ? "--dry-run" : "--force"];
        if (args.include_ignored) parts.push("-x");
        if (args.directories_only || paths.length > 0) parts.push("-d");
        return runGit(`${parts.join(" ")}${buildGitPathspecSuffix(cwd, paths)}`);
      }
      ...
      if (action === "auth_status") {
        ...
        const command = `gh auth status --hostname ${quote(args.hostname as string)}`;
        return buildManagedCommandResponse(ctl, command, { cwd: workspaceRoot, shell, env }, timeoutMs, maxOutputBytes);
      }
      if (action === "login") {
        if (!await commandAvailable("gh", workspaceRoot, shell, env, timeoutMs, maxOutputBytes)) {
        ...
        if (tokenVar) {
          ...
          if (!token) throw new Error(`Environment variable '${tokenVar}' is not set.`);
...
`;
          ...
          return buildCommandResponse(`gh auth login --hostname ${args.hostname as string} --git-protocol ${args.git_protocol as string} --with-token`, result);
        }
        if (!args.use_web) throw new Error("Provide token_env_var or set use_web=true.");
        const command = `gh auth login --hostname ${quote(args.hostname as string)} --git-protocol ${quote(args.git_protocol as string)} --web`;
...
tools.push(tool({
    ...
    implementation: safeTool("as_code_outline", async (params) => {
      ...
      for (const target of targets) {
        ...
        const detectedLanguage = detectOutlineLanguage(target.fullPath, language as string);
        const symbols = buildCodeOutline(text, detectedLanguage, limit as number);
        results.push({
...
tools.push(tool({
    ...
    implementation: safeTool("as_dynamic_tool", async ({ action, name, description, args_schema, python_code, example_args_json, args_json, overwrite, detail }) => {
      ...
      if (selectedAction === "list") {
        if (!await fileExists(toolsDir)) return json({ tools: [] });
        ...
        if (detailLevel === "max") return json({ detail: detailLevel, tools: toolsList });
      ...
      if (selectedAction === "scaffold") {
        const parsedExample = JSON.parse(example_args_json as string);
        ...
        const toolPath = path.join(toolsDir, `${name as string}.json`);
        if (await fileExists(toolPath) && !overwrite) throw new Error(`Tool '${name as string}' already exists.`);
      ...
      if (selectedAction === "create") {
        if ((args_schema as string).trim()) JSON.parse(args_schema as string);
        await fsp.mkdir(toolsDir, { recursive: true });
        const toolPath = path.join(toolsDir, `${name as string}.json`);
        if (await fileExists(toolPath) && !overwrite) throw new Error(`Tool '${name as string}' already exists.`);
        await fsp.writeFile(toolPath, JSON.stringify({
      ...
      if (selectedAction === "validate") {
        const toolPath = resolveInsideDirectory(toolsDir, `${name as string}.json`);
        if (!await fileExists(toolPath)) throw new Error(`Tool '${name as string}' not found.`);
      ...
      if (selectedAction === "call") {
        requireCommandExecution();
        JSON.parse(args_json as string);
        const toolPath = resolveInsideDirectory(toolsDir, `${name as string}.json`);
        if (!await fileExists(toolPath)) throw new Error(`Tool '${name as string}' not found.`);
        const toolDef = JSON.parse(await fsp.readFile(toolPath, "utf8"));
        ...
        try {
          ...
          await fsp.writeFile(argsPath, args_json as string, "utf8");
          const script = [
            "import json",
            "import io",
            "import inspect",
            ...
            `    _named = locals().get(${JSON.stringify(name as string)})`,
            "    if callable(_named):",
          ...
          const parsedOutput = result.stdout ? (() => { try { return JSON.parse(result.stdout); } catch { return null; } })() : null;
          const base = {
            tool: name,
            success: !result.error && result.exitCode === 0,
          ...
          const withResult = await compactValueResult(
            "as_dynamic_tool",
            detailLevel,
            "result",
            parsedOutput?.result ?? null,
            base,
          );
          return json(await compactValueResult(
            "as_dynamic_tool",
            detailLevel,
            "output",
            parsedOutput?.output ?? null,
            withResult,
          ));
        } finally {
...
tools.push(tool({
    ...
    implementation: safeTool("consult_secondary_agent", async ({ task, agent_role, context: extraContext, allow_tools, working_directory }) => {
      const enabled = (ctl.getPluginConfig(configSchematics).get("enableSecondaryAgent") as boolean | undefined) ?? false;
      if (!enabled) {
        throw new Error("Secondary agent is disabled in agentic-studio settings.");
      }
      const maxDepth = (ctl.getPluginConfig(configSchematics).get("subAgentMaxDepth") as number | undefined) ?? 2;
      ...
      const preferredModelId = (ctl.getPluginConfig(configSchematics).get("subAgentModelId") as string | undefined)?.trim() || "";
      ...
      const workingDir = resolveInsideWorkspace(workspaceRoot, working_directory as string);
      const subAgentProfiles = subAgentRuntime.getSubAgentProfiles(ctl);
      const debugEnabled = (ctl.getPluginConfig(configSchematics).get("enableAutoDebug") as boolean | undefined) ?? false;
      const maxIterations = (ctl.getPluginConfig(configSchematics).get("subAgentMaxIterations") as number | undefined) ?? 6;
      const showFullCode = (ctl.getPluginConfig(configSchematics).get("subAgentShowFullCode") as boolean | undefined) ?? false;
      ...
      try {
        ...
        if (instructions.trim()) baseSystemPrompt = instructions.trim();
      } catch {
        // ignore
      }
      ...
      try {
        for (let iteration = 0; iteration < maxIterations; iteration++) {
          ...
          if (!message) throw new Error("Sub-agent returned an empty response.");
      ...
      if (debugEnabled && permissionLevel === "full" && filesModified.length > 0 && role !== "reviewer" && subAgentRuntime.getDepth() < maxDepth) {
        ...
        if (debugMessages.length > 0) {
          const reviewResult = await (async () => {
            subAgentRuntime.incrementDepth();
            try {
              const reviewMessages: SubAgentMessage[] = [
...
tools.push(tool({
    ...
    implementation: safeTool("as_run_shell_command", async ({ command, directory }) => {
      requireCommandExecution();
      const cwd = resolveInsideWorkspace(workspaceRoot, directory as string);
      return buildManagedCommandResponse(ctl, command as string, { cwd, shell, env }, timeoutMs, maxOutputBytes);
    }),
  }));
}
