...
function registerFilesTools(ctx, tools) {
    ...
    const normalizeFsLookup = (value) => {
        const normalized = path.normalize(String(value || ""));
        return process.platform === "win32" ? normalized.toLowerCase() : normalized;
    };
    const scheduleEmbedCleanup = async (workspaceFilePath) => {
    ...
    const resolveTargets = (params, options = {}) => resolveBatchFileTargets({
        workspaceRoot,
    ...
    const normalizeLmStudioLocalPath = (value) => {
    ...
    const compactFileEntry = (baseDirectory, currentPath, entry) => ({
        path: path.relative(baseDirectory, currentPath) || ".",
        name: entry.name,
        type: entry.isDirectory() ? "directory" : entry.isSymbolicLink() ? "symlink" : "file",
    });
    const compactPathDescription = (description) => Object.fromEntries(Object.entries({
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_file_tree", async ({ directory, folder_list, max_depth, max_entries, detail }) => {
            ...
            for (const entry of directories) {
                const startDir = resolveInsideWorkspace(workspaceRoot, entry);
                if (!await pathIsDirectory(startDir)) {
                    throw new Error("The requested path is not a directory.");
                }
                const treeLines = await buildDirectoryTree(startDir, workspaceRoot, max_depth, max_entries, detailLevel === "compact" ? startDir : workspaceRoot);
                results.push({
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_file_list", async ({ directory, folder_list, recursive, limit, detail }) => {
            ...
            const detailLevel = normalizeDetailLevel(detail);
            const results = [];
            const visit = async (baseDir, currentDir) => {
            ...
            for (const entry of directories) {
                ...
                const startDir = String(entry) === ":workspace_trash:"
                    ...
                    : resolveInsideWorkspace(workspaceRoot, entry);
                if (!await pathIsDirectory(startDir)) {
                    throw new Error("The requested path is not a directory.");
                }
                    ...
                    if (exiftoolPath) {
                        ...
                        if (!exifResult.error && exifResult.stdout) {
                            ...
                            for (const entry of Array.isArray(parsed) ? parsed : []) {
                                ...
                                if (source)
                                    bySource.set(normalizeFsLookup(source), entry);
                            }
                            for (const result of results) {
                                ...
                                for (const [key, value] of Object.entries(extra)) {
                                    if (key === "SourceFile" || key in result)
                                        continue;
                                    result[key] = value;
                                }
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_file_mkdir", async ({ path: relPath }) => {
            const fullPath = resolveInsideWorkspace(workspaceRoot, relPath);
            if (isTestMode()) {
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_file_copy", async (params) => {
            ...
            const destinationPath = resolveInsideWorkspace(workspaceRoot, destination);
            ...
            for (const target of sources) {
                ...
                await fsp.cp(target.fullPath, targetDestination, { recursive: true, force: overwrite, errorOnExist: !overwrite });
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_file_set_times", async (params) => {
            ...
            for (const target of targets) {
                ...
                const modified = modified_iso ? new Date(modified_iso) : currentStat.mtime;
                const accessed = accessed_iso ? new Date(accessed_iso) : currentStat.atime;
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_file_link", async (params) => {
            ...
            const linkPath = resolveInsideWorkspace(workspaceRoot, link_path);
            ...
            for (const entry of targets) {
                ...
                if (link_type === "hardlink") {
                    ...
                    await fsp.symlink(entry.fullPath, destinationLink, symlinkType);
                }
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_file_trash_restore", async ({ trash_name, destination, overwrite }) => {
            const trashPath = resolveInsideDirectory(workspaceTrashDirectory(workspaceRoot), trash_name);
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_file_operations_log", async ({ limit }) => {
            const entries = await readOperationLog(workspaceRoot, limit);
            return json({ entries });
        }),
    }));
    tools.push(tool({
        ...
        implementation: safeTool("as_file_undo", async ({ index, indexes }) => {
            ...
            const applyUndo = async (historyIndex, last) => {
                ...
                if (strategy === "move_back") {
                    ...
                    const destinationPath = resolveInsideWorkspace(workspaceRoot, String(undo.destination || ""));
                    if (!await fileExists(trashPath))
                        throw new Error("Trash source for undo no longer exists.");
                    ...
                    const moves = Array.isArray(undo.moves) ? undo.moves : [];
                    const applied = [];
                    ...
                    const renames = Array.isArray(undo.renames) ? undo.renames : [];
                    const reversed = [];
            ...
            for (const selected of selectedEntries) {
                results.push(await applyUndo(selected.index, selected.entry));
            }
            return json({
        ...
        implementation: safeTool("as_file_rename", async ({ directory, paths_json, file_list, folder_list, file_pattern, file_pattern_flags, folder_recursive, recursive, include_files, include_directories, include_hidden, file_extensions, match_pattern, match_regex, rules_json, sort_by, sort_direction, collision_strategy, invalid_character_strategy, invalid_character_replacement, preview, limit }) => {
            const parseRules = (raw) => {
                ...
                try {
                    ...
                    throw new Error(`rules_json must be valid JSON: ${error.message}`);
                }
            ...
            const pad = (value, width) => String(value).padStart(Math.max(0, width), "0");
            ...
            const toWords = (value) => stripDiacritics(value).trim().split(/[^A-Za-z0-9]+/).filter(Boolean);
            const caseTransform = (value, mode) => {
                if (mode === "lower")
                    return value.toLowerCase();
                if (mode === "upper")
                    return value.toUpperCase();
                if (mode === "title")
                    return value.toLowerCase().replace(/\b([a-z0-9])/g, (match) => match.toUpperCase());
                if (mode === "sentence")
                    return value.toLowerCase().replace(/^(\s*[a-z0-9])/, (match) => match.toUpperCase());
                if (mode === "camel") {
            ...
            const normalizeName = (value, style) => {
                let next = stripDiacritics(value);
                if (style === "spaces")
                    return next.replace(/[_-]+/g, " ").replace(/\s+/g, " ").trim();
                if (style === "ascii")
                    return next.replace(/[^\x20-\x7E]+/g, "").trim();
                if (style === "lower")
                    return next.toLowerCase();
                if (style === "safe")
                    return next.replace(/[<>:"/\\|?*\x00-\x1F]+/g, "-").replace(/\s+/g, " ").trim();
                return next
                    .toLowerCase()
                    .replace(/&/g, " and ")
                    .replace(/[<>:"/\\|?*\x00-\x1F]+/g, "-")
            ...
            const applyReplacement = (value, rule, replacementOverride) => {
            ...
            const sanitizeInvalidFileNameCharacters = (value) => {
            ...
            const rules = parseRules(rules_json);
            const rootDir = resolveInsideWorkspace(workspaceRoot, directory);
            ...
            const matcher = String(match_pattern || "").trim()
                ? (match_regex
                    ? new RegExp(match_pattern)
                    : null)
                : null;
            const selectionMatcher = String(file_pattern || "").trim()
                ? new RegExp(file_pattern, String(file_pattern_flags || "").replace(/[^dgimsuy]/g, ""))
                : null;
            const seenSources = new Set();
            const candidates = [];
            const addCandidate = async (fullPath) => {
                ...
                if (isDirectory && !include_directories)
                    return;
                if (!isDirectory && !include_files)
                    return;
                const relPath = path.relative(workspaceRoot, resolved);
                const base = path.basename(resolved);
                if (!include_hidden && base.startsWith("."))
                    return;
                if (!isDirectory && extensionSet.size > 0 && !extensionSet.has(path.extname(base).toLowerCase()))
                    return;
                if (selectionMatcher) {
            ...
            if (hasExplicitSelection) {
                ...
                if (!await pathIsDirectory(rootDir))
                    throw new Error("directory must point to an existing directory when paths_json is not provided.");
                if (include_files) {
                    for (const filePath of await collectFiles(rootDir, limit, [], recursive)) {
                        await addCandidate(filePath);
                    }
                }
                if (include_directories) {
                    const remaining = Math.max(0, limit - candidates.length);
                    for (const directoryPath of await collectDirectories(rootDir, remaining, [], recursive)) {
            ...
            candidates.sort((left, right) => {
                let comparison = 0;
                if (sort_by === "name")
                    comparison = left.base.localeCompare(right.base);
                else if (sort_by === "created")
                    comparison = left.stat.birthtimeMs - right.stat.birthtimeMs;
                else if (sort_by === "modified")
                    comparison = left.stat.mtimeMs - right.stat.mtimeMs;
                else if (sort_by === "size")
                    comparison = left.stat.size - right.stat.size;
                else
                    comparison = left.relPath.localeCompare(right.relPath);
                return comparison * sortMultiplier || left.relPath.localeCompare(right.relPath);
            });
            const renderTemplate = (template, candidate, sequenceIndex, stem, ext) => {
                ...
                return String(template).replace(/\{([A-Za-z_]+)(?::([^}]+))?\}/g, (_match, token, arg) => {
                    ...
                    if (key === "category")
                        return category;
                    if (key === "created")
                        return formatDate(candidate.stat.birthtime, String(arg || "yyyyMMdd-HHmmss"));
                    if (key === "modified")
                        return formatDate(candidate.stat.mtime, String(arg || "yyyyMMdd-HHmmss"));
                    if (key === "accessed")
                        return formatDate(candidate.stat.atime, String(arg || "yyyyMMdd-HHmmss"));
                    if (key === "now")
                        return formatDate(new Date(), String(arg || "yyyyMMdd-HHmmss"));
                    return _match;
                });
            };
            const applyRules = (candidate, sequenceIndex) => {
                ...
                const transformScoped = (rule, transform) => {
                ...
                for (const rule of rules) {
                    ...
                    if (!type)
                        throw new Error("Each rename rule must include a type or method.");
                    if (type === "new_name" || type === "template" || type === "list") {
                        if (type === "list") {
                            const values = Array.isArray(rule.values) ? rule.values : [];
                            if (sequenceIndex - 1 < values.length)
                                setScoped("base", String(values[sequenceIndex - 1]));
                        }
                        else {
                        ...
                        transformScoped(rule, (value) => {
                            ...
                            if (side === "left")
                                return value.replace(left, "");
                            if (side === "right")
                                return value.replace(right, "");
                        ...
                        if (!candidate.isDirectory) {
                            ...
                            if (mode === "lower")
                                ...
                                setScoped("extension", String(rule.value ?? rule.text ?? ""));
                        }
                    }
                ...
                if (!nextName || nextName === "." || nextName === "..")
                    throw new Error(`Rule set produced an invalid empty name for ${candidate.relPath}.`);
                if (/[\/\\]/.test(nextName))
                    throw new Error(`Rule set produced a path separator in '${nextName}' for ${candidate.relPath}.`);
                if (/[<>:"|?*\x00-\x1F]/.test(nextName))
                    throw new Error(`Rule set produced Windows-invalid filename characters in '${nextName}' for ${candidate.relPath}.`);
                if (/[. ]$/.test(nextName))
                    throw new Error(`Rule set produced a Windows-invalid trailing dot or space in '${nextName}' for ${candidate.relPath}.`);
            ...
            const skipped = [];
            const uniqueDestination = async (destinationPath, sourcePath) => {
                ...
                while ((await fileExists(nextPath) && nextPath.toLowerCase() !== sourceKey) || destinationKeys.has(nextPath.toLowerCase())) {
                    if (collision_strategy === "error")
                        throw new Error(`Destination already exists: ${path.relative(workspaceRoot, nextPath)}`);
            ...
            const adjusted = planned.map((rename) => {
                ...
                for (const move of directoryMoves) {
                    if (move.sourcePath === rename.sourcePath)
                        continue;
            ...
            const nestedDirectoryRename = adjusted.find((rename) => rename.isDirectory && adjusted.some((other) => (other.sourcePath !== rename.sourcePath
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_file_watch_create", async ({ watch_id, directory, recursive, include_hidden, limit }) => {
            const targetDir = resolveInsideWorkspace(workspaceRoot, directory);
            const snapshot = await captureDirectorySnapshot(targetDir, workspaceRoot, recursive, include_hidden, limit);
            const watcher = {
                id: watch_id,
        ...
        implementation: safeTool("as_file_watch_list", async () => {
            ...
            for (const entry of entries) {
                if (!entry.endsWith(".json"))
                    continue;
                try {
                    const watcher = JSON.parse(await fsp.readFile(path.join(dir, entry), "utf8"));
                    watchers.push({
                        id: watcher.id,
                        directory: watcher.directory,
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_file_watch_scan", async ({ watch_id, refresh_snapshot, limit }) => {
            const watcher = await readWatcher(workspaceRoot, watch_id);
            const currentSnapshot = await captureDirectorySnapshot(resolveInsideWorkspace(workspaceRoot, watcher.directory), workspaceRoot, watcher.recursive, watcher.includeHidden, limit);
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_file_watch_remove", async ({ watch_id }) => {
            const watcherPath = resolveInsideDirectory(fileWatchersDirectory(workspaceRoot), `${watch_id}.json`);
            if (isTestMode()) {
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_file_plan_reorganization", async ({ source_directory, destination_root, limit, plan_name }) => {
            const sourceDir = resolveInsideWorkspace(workspaceRoot, source_directory);
            const destinationRoot = resolveInsideWorkspace(workspaceRoot, destination_root);
            const files = await collectFiles(sourceDir, limit);
            const planEntries = [];
            for (const filePath of files) {
                ...
                if (relativeSource === relativeDestination)
                    continue;
                planEntries.push({
            ...
            const planId = plan_name.trim() || `plan-${Date.now()}`;
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_file_preview_moves", async ({ plan_id, limit }) => {
            const planPath = resolveInsideDirectory(reorgPlansDirectory(workspaceRoot), `${plan_id}.json`);
            ...
            return json({
                ...
                destinationRoot: plan.destinationRoot,
                entryCount: Array.isArray(plan.entries) ? plan.entries.length : 0,
                entries: Array.isArray(plan.entries) ? plan.entries.slice(0, limit) : [],
            });
        }),
    }));
    tools.push(tool({
        ...
        implementation: safeTool("as_file_apply_reorganization", async ({ plan_id, overwrite, limit }) => {
            ...
            if (!await fileExists(planPath))
                throw new Error("Plan not found.");
            const plan = JSON.parse(await fsp.readFile(planPath, "utf8"));
            const entries = Array.isArray(plan.entries) ? plan.entries.slice(0, limit) : [];
            ...
            await appendOperationLog(workspaceRoot, {
                ...
                undo: {
                    ...
                    moves: applied.map((entry) => ({
                        ...
                        destination: String(entry.source || ""),
                    })),
                },
            });
            return json({
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_file_resolve_duplicates", async ({ files_json, file_list, folder_list, file_pattern, file_pattern_flags, folder_recursive, include_hidden, file_limit, keep_path, action, destination_directory, overwrite, confirmed }) => {
            ...
            const normalizeDuplicatePath = (value) => String(value || "").replace(/\\/g, "/").trim();
            const keepSet = new Set([normalizeDuplicatePath(String(keep_path))]);
            const targets = files.filter((value) => !keepSet.has(normalizeDuplicatePath(value)));
            if (isTestMode()) {
                const dryRunResults = [];
                for (const relPath of targets) {
                    ...
                    if (action === "move") {
                        const destinationDir = resolveInsideWorkspace(workspaceRoot, destination_directory);
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_file_patch", async ({ path: relPath, file_list, folder_list, file_pattern, file_pattern_flags, folder_recursive, include_hidden, file_limit, operation, old_text, new_text, pattern, replacement, content, patches_json, preview, encoding, newline, create_if_missing, replace_all, case_sensitive, line, start_line, end_line, ensure_final_newline }) => {
            const inferNewline = (text) => text.includes("\r\n") ? "\r\n" : "\n";
            const normalizeOutputNewline = (text, original) => {
            ...
            const explicitPatches = String(patches_json || "").trim()
                ? (() => {
                    const parsed = JSON.parse(patches_json);
                    if (!Array.isArray(parsed) || parsed.length === 0)
                        throw new Error("patches_json must be a non-empty JSON array.");
                    return parsed;
                })()
                : [];
            const patches = [...explicitPatches, ...selectedSimplePatches];
            if (patches.length === 0)
                throw new Error("Provide path, file_list, folder_list, or patches_json.");
            ...
            for (const [index, rawPatch] of patches.entries()) {
                ...
                if (!patchPath)
                    throw new Error(`Patch ${index + 1} is missing path.`);
                if (!byPath.has(patchPath))
                    byPath.set(patchPath, []);
            ...
            for (const [patchPath, filePatches] of byPath.entries()) {
                ...
                const original = exists ? await fsp.readFile(fullPath, encoding) : "";
                ...
                for (const [index, patch] of filePatches.entries()) {
                    ...
                    if (op === "replace") {
                        ...
                        if (!oldText)
                            throw new Error(`Patch ${index + 1} old_text must not be empty.`);
                        const count = next.split(oldText).length - 1;
                        if (count === 0)
                            throw new Error(`Patch ${index + 1} old_text not found in ${patchPath}.`);
                        const all = Boolean(patch.replace_all ?? replace_all);
                        if (!all && count !== 1)
                            throw new Error(`Patch ${index + 1} matched ${count} times in ${patchPath}; make it unique or set replace_all=true.`);
                    ...
                    if (op === "regex_replace") {
                        ...
                        if (!regexPattern)
                            throw new Error(`Patch ${index + 1} pattern must not be empty.`);
                        ...
                        if (matches.length === 0)
                            throw new Error(`Patch ${index + 1} pattern did not match ${patchPath}.`);
                        if (!Boolean(patch.replace_all ?? replace_all) && matches.length !== 1)
                            throw new Error(`Patch ${index + 1} matched ${matches.length} times in ${patchPath}; make it unique or set replace_all=true.`);
                    ...
                    if (op === "delete_range") {
                        ...
                        if (end < start)
                            throw new Error(`Patch ${index + 1} end_line must be >= start_line.`);
                        const lines = next.split(/\r?\n/);
                ...
                if (!effectivePreview && next !== original) {
                    ...
                    await fsp.writeFile(fullPath, next, encoding);
                }
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_file_fuzzy_find", async ({ query, directory, file_list, folder_list, file_pattern, file_pattern_flags, folder_recursive, include_hidden, file_limit, limit, extensions }) => {
            ...
            for (const filePath of files) {
                ...
                if (extSet.size > 0 && !extSet.has(path.extname(rel).toLowerCase()))
                    continue;
                const score = Math.max(computeFuzzyScore(query, path.basename(rel)), computeFuzzyScore(query, rel));
                if (score <= 0.15)
                    continue;
                const stat = await fsp.stat(filePath);
                ranked.push({
            ...
            return json({ query, totalMatches: ranked.length, results: ranked.slice(0, limit) });
        }),
    }));
    tools.push(tool({
        ...
        implementation: safeTool("as_file_find_duplicates", async ({ directory, file_list, folder_list, file_pattern, file_pattern_flags, folder_recursive, include_hidden, file_limit, algorithm, limit }) => {
            ...
            const files = (Array.isArray(file_list) && file_list.length > 0) || (Array.isArray(folder_list) && folder_list.length > 0)
                ...
                : await collectFiles(startDir, limit);
            const crypto = await Promise.resolve().then(() => __importStar(require("crypto")));
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_file_embed", async (params) => {
            ...
            const externalTargetKeys = new Set();
            const maybeCollectExternalFile = async (candidate) => {
                const normalizedCandidate = String(candidate || "").trim();
                if (!normalizedCandidate || /^https?:\/\//i.test(normalizedCandidate) || !path.isAbsolute(normalizedCandidate))
                    return false;
                const normalizedLookup = normalizeFsLookup(normalizedCandidate);
                if (normalizedLookup === normalizedWorkspaceRoot || normalizedLookup.startsWith(`${normalizedWorkspaceRoot}${path.sep}`))
                    return false;
            ...
            for (const entry of normalizedFileList) {
                if (!await maybeCollectExternalFile(entry))
                    filteredFileList.push(entry);
            }
            ...
            const buildLmStudioImageUrl = (fullPath) => {
            ...
            const buildFileLinkUrl = (fullPath) => `file:///${fullPath.replace(/\\/g, "/")}`;
            const embedCacheDirectory = path.join(workspaceRoot, ".lmstudio-embed-cache");
            ...
            for (const target of targets) {
                ...
                if (imageExtensions.has(ext)) {
                    ...
                    try {
                        const maxSize = params.max_text_size;
                        ...
                        markdownOutput += `\n\n**File: ${basename}** *(error reading file: ${error.message})*\n\n`;
                        results.push({
                            path: relPath,
                            type: "text",
                            embedded: false,
            ...
            const embeddedImageCount = results.filter((entry) => entry?.type === "image" && entry?.embedded === true).length;
            return json({
                success: true,
                count: results.length,
