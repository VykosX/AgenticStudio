...
const consolidated_1 = require("./consolidated");
const mathAndUnits_1 = require("./mathAndUnits");
function schemaParameterSummary(schema) {
    const properties = schema?.properties || {};
    return Object.entries(properties).map(([name, value]) => ({
        name,
        type: value?.type || (Array.isArray(value?.anyOf) ? value.anyOf.map((entry) => entry?.type).filter(Boolean) : null),
...
function genericToolUseCases(toolName, category) {
...
function genericToolTips(toolName, category) {
    const normalizedName = String(toolName || "").toLowerCase();
    const normalizedCategory = String(category || "").toLowerCase();
    if (normalizedName === "as_math") {
        return [
...
function genericToolExamples(toolName, category) {
    const normalizedName = String(toolName || "").toLowerCase();
    const normalizedCategory = String(category || "").toLowerCase();
    if (normalizedName === "as_math") {
        return [
    ...
    if ((0, consolidated_1.isToolAliasName)(normalizedName)) {
        const alias = (0, consolidated_1.findToolAliasDefinition)(normalizedName);
        return [
...
function buildToolSpecificReference(toolDoc, detailLevel) {
    const includeDetailedValues = detailLevel === "standard" || detailLevel === "detailed";
    switch (toolDoc?.name) {
        case "as_math":
            return (0, mathAndUnits_1.getMathToolReference)(includeDetailedValues);
        case "as_unit_conversion":
            return (0, mathAndUnits_1.getUnitConversionToolReference)(includeDetailedValues);
        default:
            return null;
    }
}
function actionEnumValuesFromSchema(schema) {
    const actionProperty = schema?.properties?.action;
    if (!actionProperty)
        return [];
    if (Array.isArray(actionProperty.enum)) {
        return actionProperty.enum.map((value) => String(value));
    }
    if (Array.isArray(actionProperty.anyOf)) {
        return actionProperty.anyOf
            .flatMap((entry) => Array.isArray(entry?.enum) ? entry.enum : [])
            .map((value) => String(value));
    }
    return [];
}
function controllerActionExamples(toolName) {
    switch (toolName) {
...
function buildActionExamples(toolName, runtimeSchemaValue, detailLevel) {
    const examplesByAction = controllerActionExamples(toolName);
    const actions = actionEnumValuesFromSchema(runtimeSchemaValue);
    if (actions.length === 0 || Object.keys(examplesByAction).length === 0)
        return [];
...
function buildFileSelectionGuidance(runtimeSchemaValue, detailLevel) {
...
function buildDetailedToolHelp(toolDoc, runtimeSchemaValue, detailLevel) {
...
function registerMetaTools(ctx, tools) {
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_caveman_status", async ({ action }) => {
            ...
            if (normalizedAction === "skip_next_tool") {
                ...
                cavemanControl.effectiveCavemanMode = (ctl.getPluginConfig(configSchematics).get("cavemanSkillProfile") || "normal");
    ...
    if (allowIndividualToolRequests) {
        tools.push(tool({
            ...
            implementation: safeTool("as_request_tool", async ({ action, tool_name }) => {
                ...
                const docs = await discoverToolDocumentation("own", enabledPlugins, "live_only", tools, currentToolProfile, "full", currentRequestedToolNames, allowIndividualToolRequests);
                const liveToolNames = new Set(docs
                ...
                const nextRequestedTools = normalizedAction === "disable"
                    ? currentRequestedToolNames.filter((name) => !matchedToolNames.includes(name))
                    : Array.from(new Set([
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_tool_catalog", async ({ scope, profile, category, query, detailed, query_tool, write_to_file, limit }) => {
            ...
            let docs = await discoverToolDocumentation(normalizedScope, enabledPlugins, normalizedScope === "own" || normalizedScope === "enabled" ? "live_only" : "all_installed", tools, currentToolProfile, discoveryProfile, currentRequestedToolNames, allowIndividualToolRequests);
            if (category.trim()) {
            ...
            if (query.trim()) {
                ...
                docs = docs.filter((doc) => doc.name.toLowerCase().includes(wanted)
            ...
            const slicedTools = slicedDocs.filter((doc) => !(0, consolidated_1.isToolAliasName)(doc.name));
            const slicedAliases = slicedDocs.filter((doc) => (0, consolidated_1.isToolAliasName)(doc.name));
            ...
            const payload = matchedTools.length > 0
                ? {
                    ...
                    individuallyRequestedTools: currentRequestedToolNames,
                    enabledPlugins,
                    aliasesEnabled: (0, consolidated_1.aliasesEnabled)(ctl),
            ...
            const effectiveWritePath = resolveDefaultToolOutputPath(write_to_file, "tool-catalog", detailed ? "standard" : "summary");
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_tool_help", async ({ goal, tool_name, tool_names, scope, profile, detail_level, write_to_file }) => {
            ...
            const normalizedScope = normalizeMetaScope(scope);
            const discoveryProfile = recommendationDiscoveryProfile(normalizedScope, requestedProfile);
            const docs = await discoverToolDocumentation(normalizedScope, enabledPlugins, normalizedScope === "own" || normalizedScope === "enabled" ? "live_only" : "all_installed", tools, currentToolProfile, discoveryProfile, currentRequestedToolNames, allowIndividualToolRequests);
            ...
            if (requestedToolNames.length > 0) {
                const toolResults = requestedToolNames.map((requestedName) => {
                    const aliasDefinition = (0, consolidated_1.findToolAliasDefinition)(requestedName);
                    ...
                    return mergeDefined({
                        ...
                        aliasesEnabled: aliasDefinition ? (0, consolidated_1.aliasesEnabled)(ctl) : undefined,
                        generalCall: aliasDefinition ? aliasDefinition.generalCall : undefined,
                        help: buildDetailedToolHelp(matchedTool, schema, normalizedDetailLevel),
                ...
                const effectiveWritePath = resolveDefaultToolOutputPath(write_to_file, `${requestedToolNames[0]}-help`, normalizedDetailLevel);
                const writtenPath = await maybeWriteToolOutputToFile(workspaceRoot, effectiveWritePath, payload);
            ...
            const recommendations = recommendToolsForGoal(goal, docs);
            const payload = {
                goal,
                enabledPlugins,
                detailLevel: normalizedDetailLevel,
            ...
            const effectiveWritePath = resolveDefaultToolOutputPath(write_to_file, "tool-help", normalizedDetailLevel);
            const writtenPath = await maybeWriteToolOutputToFile(workspaceRoot, effectiveWritePath, payload);
            return json(mergeDefined({
                goal,
                enabledPlugins,
