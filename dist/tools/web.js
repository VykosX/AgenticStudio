...
function registerWebTools(ctx, tools) {
    ...
    const compactSearchRecord = (entry) => compactObject(entry, [
    ...
    const compactTorrentRecord = (entry) => compactObject(entry, [
    ...
    const compactTorrentFileRecord = (entry) => compactObject(entry, [
        "index", "name", "size", "progress", "priority", "is_seed", "piece_range", "availability",
    ]);
    const encodeInlineNodeLiteral = (value) => Buffer.from(String(value || ""), "utf8").toString("base64");
    const compactTextPayload = async (toolName, detailLevel, label, text, metadata = {}) => {
        ...
        if (detailLevel === "compact" && String(text || "").length > preview.length) {
            payload[`${label}ReportPath`] = await maybeWriteToolOutputToFile(workspaceRoot, createReportPath(toolName, label), { ...metadata, [label]: text });
            payload.reportReadGuidance = REPORT_READ_GUIDANCE;
        }
        return payload;
    };
    const compactCollectionPayload = async (toolName, detailLevel, payload, key, values, mapper = (entry) => entry) => {
        if (detailLevel === "max")
            return payload;
        const normalized = values.map((entry) => mapper((entry && typeof entry === "object") ? entry : { value: entry }));
        ...
        if (detailLevel === "compact" && normalized.length > sample.length) {
            result[`${key}ReportPath`] = await maybeWriteToolOutputToFile(workspaceRoot, createReportPath(toolName, key), payload);
    ...
    const isProbablyDownloadableUrl = (value) => {
    ...
    const inferFileNameFromUrl = (value, fallback = "download.bin") => {
        const parsed = safeUrl(value);
    ...
    const contentTypeLooksHtml = (value) => {
        const type = String(value || "").toLowerCase();
        return !type || type.includes("text/html") || type.includes("application/xhtml");
    };
    const resolveDownloadDestination = async (outputPath, suggestedFileName) => {
    ...
    const transformKnownDownloadUrl = (value) => {
        const parsed = safeUrl(value);
        if (!parsed)
    ...
    const extractGoogleDriveConfirmUrl = (html, baseUrl) => {
    ...
    const scoreDownloadCandidate = (candidateUrl, label, contentType = "", contentLength = "") => {
        let score = 0;
        const ext = extensionFromUrl(candidateUrl);
        const lowerLabel = String(label || "").toLowerCase();
        const lowerType = String(contentType || "").toLowerCase();
        if (downloadableExtensions.has(ext))
            score += 60;
        if ([".pdf", ".epub", ".zip", ".mp3", ".mp4", ".torrent"].includes(ext))
            score += 15;
        if (/\b(download|get|file|pdf|epub|source|original|full text|archive)\b/i.test(lowerLabel))
            score += 20;
        if (/application\/(pdf|zip|octet-stream|epub)|audio\/|video\//i.test(lowerType))
            score += 35;
        if (contentLength && Number(contentLength) > 0)
            score += 5;
        return score;
    };
    const probeCandidateHeaders = async (candidateUrl) => {
        try {
    ...
    const discoverDownloadOptions = async (pageUrl, html, limit) => {
        ...
        for (const match of matches) {
            ...
            if (baseScore < 20 && !isProbablyDownloadableUrl(absolute))
                continue;
            seen.add(absolute);
            ...
            if (candidates.length >= Math.max(limit * 3, 12))
                break;
        }
        const probed = [];
        for (const candidate of candidates.slice(0, Math.max(limit * 2, 8))) {
            const headers = await probeCandidateHeaders(candidate.url);
    ...
    const fetchDownloadToFile = async (downloadUrl, outputPath, overwrite, timeoutMsForCall, inheritedNote = null) => {
        ...
        try {
            ...
            if (contentTypeLooksHtml(contentType) && /drive\.google\.com/i.test(response.url || downloadUrl)) {
                ...
                if (confirmUrl) {
                    ...
                    if (!response.ok)
                        throw new Error(`Google Drive confirmed download failed with status ${response.status}: ${response.statusText}`);
                    contentType = response.headers.get("content-type") || "";
                }
                else {
    ...
    const runVideoDownloadRedirect = async (downloadUrl, outputPath, overwrite, timeoutMsForCall) => {
    ...
    const googleWebSearch = async (searchQuery, limit) => {
    ...
    const decodeBingClickUrl = (rawUrl) => {
    ...
    const bingWebSearch = async (searchQuery, limit) => {
    ...
    const normalizeSiteToken = (siteValue) => {
        const raw = String(siteValue || "").trim();
        if (!raw)
            return "";
        if (/\bsite:/i.test(raw) || /\bOR\b/i.test(raw))
            return raw;
        const withoutProtocol = raw.replace(/^https?:\/\//i, "").replace(/^www\./i, "").replace(/\/+$/, "");
        return withoutProtocol;
    };
    const buildScopedQuery = (rawQuery, targetSite) => {
        const normalizedSite = normalizeSiteToken(targetSite);
        if (!normalizedSite)
            return rawQuery;
        if (/\bsite:/i.test(normalizedSite) || /\bOR\b/i.test(normalizedSite))
            return `${rawQuery} ${normalizedSite}`;
        return `${rawQuery} site:${normalizedSite}`;
    };
    const yahooWebSearch = async (searchQuery, limit) => {
        ...
        for (const match of matches) {
            ...
            const title = normalizeSearchText(match[2] || "");
            if (!url || !title || !/^https?:\/\//i.test(url) || seen.has(url))
                continue;
            seen.add(url);
            results.push({ title, url });
            if (results.length >= limit)
    ...
    const getLocalSearchCountryPreference = () => {
    ...
    const getSearxInstanceCountryCode = (instance, cidrCatalog) => {
        const ips = instance.network?.ips;
        if (!ips)
            return "";
    ...
    const getSearxMirrorCatalog = async () => {
        ...
        const instances = (catalog.instances && typeof catalog.instances === "object") ? catalog.instances : {};
        const cidrCatalog = (catalog.cidrs && typeof catalog.cidrs === "object") ? catalog.cidrs : {};
        ...
        const mirrors = Object.entries(instances)
            ...
            .map(([url, info]) => {
            const searchTiming = info?.timing;
            ...
            return {
                ...
                searchSuccess: Number((searchTiming?.search?.success_percentage) || 0),
                searchMedian: Number((searchTiming?.search?.all?.median) || Number.MAX_SAFE_INTEGER),
                version: String(info?.version || ""),
                networkType: String(info?.network_type || ""),
                tlsGrade: String((info?.tls?.grade) || ""),
                htmlGrade: String((info?.html?.grade) || ""),
            };
        })
            .filter((entry) => entry.searchSuccess > 0 && entry.networkType !== "tor")
            .sort((left, right) => (left.countryRank - right.countryRank
    ...
    const listSearxMirrors = async (index, pageSize = DEFAULT_SEARCH_PAGE_SIZE) => {
    ...
    const searchSearxMirror = async (baseUrl, searchQuery, limit, language = "all") => {
    ...
    const searxWebSearch = async (searchQuery, limit, options = {}) => {
        ...
        for (const mirror of candidateMirrors) {
            try {
                ...
                errors.push(`${mirror.url}: ${error.message}`);
            }
        }
        throw new Error(`SearxNG search failed across ${candidateMirrors.length} mirror(s): ${errors.slice(0, 5).join(" | ")}`);
    };
    const runSearchProvider = async (scopedQuery, limit, preferredProvider, options = {}) => {
            ...
            try {
                ...
                if (preferredProvider === "firecrawl") {
                    ...
                    return {
                        ...
                        warning: `Firecrawl search failed and fell back to SearxNG: ${error.message}`,
        ...
        try {
            ...
            providerErrors.push(`searxng: ${error.message}`);
        ...
        try {
            ...
            providerErrors.push(`yahoo: ${error.message}`);
        }
        try {
            ...
            providerErrors.push(`bing: ${error.message}`);
        }
        try {
            ...
            providerErrors.push(`duckduckgo: ${error.message}`);
        }
        try {
    ...
    const archiveDownloadLinks = async (identifier, maxLinks) => {
        ...
        return files
            ...
            if (!name || /(^|_)(meta|files)\.xml$/i.test(name))
                return false;
            if (/encrypted|lcp|daisy|scandata|hocr|abbyy|metadata/i.test(descriptor))
                return false;
    ...
    const searchInternetArchive = async (query, limit) => {
        const params = new URLSearchParams();
        params.set("q", query);
        for (const field of ["identifier", "title", "creator", "year", "mediatype", "description"])
        ...
        if (!response.ok)
            throw new Error(`Internet Archive search failed with status ${response.status}: ${response.statusText}`);
    ...
    const decodeHtmlEntities = (value) => String(value || "")
        ...
        .replace(/&lt;/gi, "<")
        .replace(/&gt;/gi, ">");
    const normalizeSearchText = (value) => stripHtmlToText(String(value || "")).replace(/\s+/g, " ").trim();
    const searchFetchText = async (url, init = {}, timeoutForCall = 30000) => {
        const headers = { "User-Agent": WEB_USER_AGENT, ...(init.headers || {}) };
        const response = await fetch(url, {
            ...init,
            headers,
            redirect: "follow",
            signal: init.signal || AbortSignal.timeout(timeoutForCall),
        });
    ...
    const searchFetchJson = async (url, init = {}, timeoutForCall = 30000) => {
        const { response, text } = await searchFetchText(url, init, timeoutForCall);
        try {
            return { response, data: text ? JSON.parse(text) : {} };
        }
        catch (error) {
            throw new Error(`Expected JSON but received non-JSON response from ${response.url || url}: ${error.message}`);
        }
    };
    const readConfigField = (fieldName) => ctl.getPluginConfig(configSchematics).get(fieldName);
    const getBrowserAutomationBackend = () => {
    ...
    const resolveConfigPath = (configuredPath, fallbackPath) => {
        const raw = String(configuredPath || "").trim();
        if (!raw)
            return path.resolve(pluginRoot, fallbackPath);
        const expanded = raw
    ...
    const resolveCamofoxVendorDirectory = () => path.join(pluginRoot, "dependencies", "camofox-browser");
    const resolveCamofoxHomeDirectory = () => {
    ...
    const resolveCamofoxInstallDirectory = () => {
        const homeDirectory = resolveCamofoxHomeDirectory();
        if (process.platform === "win32")
            return path.join(homeDirectory, "AppData", "Local", "camoufox", "camoufox", "Cache");
        if (process.platform === "darwin")
            return path.join(homeDirectory, "Library", "Caches", "camoufox");
        return path.join(homeDirectory, ".cache", "camoufox");
    };
    const resolveDefaultPlaywrightBrowsersDirectory = () => {
        const hostHomeDirectory = os.homedir();
        if (process.platform === "win32")
            return path.join(hostHomeDirectory, "AppData", "Local", "ms-playwright");
        if (process.platform === "darwin")
            return path.join(hostHomeDirectory, "Library", "Caches", "ms-playwright");
        return path.join(hostHomeDirectory, ".cache", "ms-playwright");
    };
    const buildCamofoxRuntimeEnv = (baseEnv) => {
        const homeDirectory = resolveCamofoxHomeDirectory();
        const runtimeEnv = { ...baseEnv, HOME: homeDirectory };
    ...
    let camofoxPreparationPromise = null;
    const ensureCamofoxRuntime = async () => {
        if (camofoxPreparationPromise)
        ...
        camofoxPreparationPromise = (async () => {
            ...
            if (!await fileExists(versionPath)) {
                const fetchResult = await executeManagedCommand(ctl, "npx camoufox-js fetch", { cwd: vendorDir, shell, env: runtimeEnv }, Math.max(timeoutMs, 30 * 60 * 1000), Math.max(maxOutputBytes, 400000));
    ...
    const executeConfiguredBrowserNodeScript = async (script, timeoutOverrideMs, backend = getBrowserAutomationBackend()) => {
        const nodeExecutable = await getNodeExecutablePath(ctl);
        if (backend === "camofox") {
            const runtime = await ensureCamofoxRuntime();
            return await executeInlineNodeScript(ctl, script, shell, runtime.env, workspaceRoot, timeoutOverrideMs, maxOutputBytes, nodeExecutable, runtime.vendorDir);
        }
        return await executeInlineNodeScript(ctl, script, shell, env, workspaceRoot, timeoutOverrideMs, maxOutputBytes, nodeExecutable);
    };
    const buildBrowserBackendPrelude = (preferredBackend) => [
    ...
    const extractTextRuns = (value) => {
        if (!value || typeof value !== "object")
            return normalizeSearchText(value);
        if (typeof value.simpleText === "string")
            return normalizeSearchText(value.simpleText);
        if (Array.isArray(value.runs)) {
            return normalizeSearchText(value.runs.map((entry) => entry?.text || "").join(""));
        }
        return "";
    };
    const collectNestedObjects = (value, keyName, output = []) => {
        if (!value || typeof value !== "object")
            return output;
        if (Array.isArray(value)) {
            for (const entry of value)
                collectNestedObjects(entry, keyName, output);
            return output;
        }
    ...
    const splitSiteSearchTerms = (queryValue) => {
    ...
    const getSearchResultUrl = (entry) => String(entry?.url || entry?.link || entry?.href || entry?.outboundUrl || entry?.hnUrl || "").trim();
    const scoreQueryMatch = (text, queryValue, terms) => {
    ...
    const normalizeLocaleTag = (value, fallback, mode) => {
    ...
    const searchWikipedia = async (queryValue, limitValue, languageValue) => {
    ...
    const searchReddit = async (queryValue, limitValue, subredditValue, sortValue, timeValue) => {
        ...
        return {
            ...
            results: ((data?.data?.children) || []).map((entry) => ({
    ...
    const searchHackerNews = async (queryValue, limitValue) => {
    ...
    const searchStackExchange = async (queryValue, limitValue, stackExchangeSiteValue) => {
    ...
    const searchWikiHow = async (queryValue, limitValue) => {
    ...
    const fetchPypiProjectMetadata = async (projectName) => {
    ...
    const searchPyPi = async (queryValue, limitValue, preferredProvider) => {
        const normalizedQuery = normalizeSearchText(queryValue).toLowerCase();
        ...
        try {
            ...
            rankedCandidates = projectNames
                ...
                if (normalizedName === normalizedQuery)
                    score += 180;
                if (exactCandidates.has(projectName) || exactCandidates.has(normalizedName))
                    score += 140;
            ...
            note = `${note} The Simple Index fetch failed, so results were recovered from a scoped web search: ${error.message}`;
        }
        if (rankedCandidates.length === 0) {
            ...
            rankedCandidates = [];
            for (const entry of providerResult.results) {
                ...
                if (!packageName || seenNames.has(packageName))
                    continue;
        ...
        const metadataResults = (await Promise.all(rankedCandidates
    ...
    const searchScopedSearchEngineResults = async (queryValue, limitValue, preferredProvider, scopedSite, keepResult, scoreEntry, providerLabel, note) => {
        const runVariant = async (providerPreference) => {
            ...
            for (const entry of results) {
                ...
                if (!parsedUrl || !keepResult(entry, parsedUrl))
                    continue;
    ...
    const runBrowserBackedWebsiteSearch = async (websiteLabel, searchUrl, limitValue, evaluationBody, waitForResultsMs = 3500, timeoutForCallMs = 180000) => {
        requireCommandExecution();
        ...
        const parsed = parseJsonObject(result.stdout, `${websiteLabel}_browser_search_result`);
        return {
    ...
    const searchQuora = async (queryValue, limitValue, preferredProvider) => {
        ...
        try {
            ...
            const parsed = parseJsonObject(result.stdout, "quora_browser_search_result");
            const directSearch = {
        ...
        const strictSearch = await searchScopedSearchEngineResults(queryValue, limitValue, preferredProvider, "site:quora.com", (_entry, parsedUrl) => {
            const host = normalizeHost(parsedUrl.hostname);
            const pathname = parsedUrl.pathname || "/";
            if (host === "quora.com")
                return pathname !== "/" && !/^\/(?:search|profile|about|careers|contact|business|spaces)\b/i.test(pathname);
            if (host === "help.quora.com")
                return /^\/hc\/en-us\/articles\//i.test(pathname);
            return false;
        }, (entry, parsedUrl, terms) => {
            const pathname = parsedUrl.pathname || "/";
            let score = scoreQueryMatch(`${entry.title || ""} ${entry.snippet || ""} ${pathname}`, queryValue, terms);
            if (/\/answer\//i.test(pathname))
                score += 35;
            else if (/^\/topic\//i.test(pathname))
                score += 18;
            else if (/^\/[^/]+$/i.test(pathname))
                score += 20;
            if (normalizeHost(parsedUrl.hostname) === "help.quora.com")
                score += 8;
            return score;
        }, "quora_scoped_search", `${note} Browser extraction failed, so a scoped public web search was used instead: ${browserError || "unknown browser error"}`);
        if (strictSearch.results.length > 0)
            return strictSearch;
        return await searchScopedSearchEngineResults(`quora ${queryValue}`, limitValue, preferredProvider, "", (_entry, parsedUrl) => {
            const host = normalizeHost(parsedUrl.hostname);
            if (host === "quora.com")
                return parsedUrl.pathname !== "/" && !/^\/(?:search|profile|about|careers|contact|business|spaces)\b/i.test(parsedUrl.pathname || "/");
            if (host === "help.quora.com")
                return /^\/hc\/en-us\/articles\//i.test(parsedUrl.pathname || "/");
            return false;
        }, (entry, parsedUrl, terms) => {
            let score = scoreQueryMatch(`${entry.title || ""} ${entry.snippet || ""} ${parsedUrl.pathname || "/"}`, queryValue, terms);
            if (/\/answer\//i.test(parsedUrl.pathname || ""))
                score += 35;
            else if (/^\/topic\//i.test(parsedUrl.pathname || ""))
                score += 18;
            else if (/^\/[^/]+$/i.test(parsedUrl.pathname || ""))
                score += 20;
            if (/quora/i.test(String(entry.title || "")))
                score += 10;
            if (normalizeHost(parsedUrl.hostname) === "help.quora.com")
                score += 8;
            return score;
        }, "quora_scoped_search", `${note} Browser extraction failed, and a broader Quora-branded query was used only after the scoped fallback returned nothing useful: ${browserError || "unknown browser error"}`);
    };
    const searchTwitterX = async (queryValue, limitValue, preferredProvider) => {
        ...
        let browserError = "";
        try {
            const directSearch = await runBrowserBackedWebsiteSearch("x", `https://x.com/search?q=${encodeURIComponent(queryValue)}&src=typed_query&f=live`, limitValue, [
                '      const normalize = (value) => String(value || "").replace(/\\s+/g, " ").trim();',
                "      const seen = new Set();",
                "      const results = [];",
                "      const anchors = Array.from(document.querySelectorAll('a[href]'));",
            ...
            if (directSearch.results.length > 0)
                return { ...directSearch, note };
            if (/\/(?:i\/flow\/login|login)\b/i.test(String(directSearch.pageUrl || "")) || /sign in to x|what'?s happening/i.test(String(directSearch.pageTitle || ""))) {
                return {
                    provider: `${String(directSearch.provider || "x_browser")}_blocked`,
                    searchUrl: String(directSearch.pageUrl || directSearch.searchUrl || ""),
                    note: `${note} X redirected the anonymous browser session to its login wall instead of exposing public search results.`,
                    results: [{
                            title: String(directSearch.pageTitle || "X login wall"),
                            url: String(directSearch.pageUrl || directSearch.searchUrl || ""),
        ...
        const strictSearch = await searchScopedSearchEngineResults(queryValue, limitValue, preferredProvider, "site:x.com OR site:twitter.com", (_entry, parsedUrl) => {
            const host = normalizeHost(parsedUrl.hostname);
            ...
            const pathname = parsedUrl.pathname || "/";
            let score = scoreQueryMatch(`${entry.title || ""} ${entry.snippet || ""} ${pathname}`, queryValue, terms);
            if (/\/status\/\d+/i.test(pathname))
                score += 45;
            else if (/^\/[^/]+$/i.test(pathname))
                score += 18;
            return score;
        }, "x_scoped_search", `${note} Browser extraction failed, so a scoped public web search was used instead: ${browserError || "unknown browser error"}`);
        if (strictSearch.results.length > 0)
            return strictSearch;
        return await searchScopedSearchEngineResults(`twitter ${queryValue}`, limitValue, preferredProvider, "", (_entry, parsedUrl) => {
            const host = normalizeHost(parsedUrl.hostname);
            ...
            let score = scoreQueryMatch(`${entry.title || ""} ${entry.snippet || ""} ${parsedUrl.pathname || "/"}`, queryValue, terms);
            if (/\/status\/\d+/i.test(parsedUrl.pathname || ""))
                score += 45;
            else if (/^\/[^/]+$/i.test(parsedUrl.pathname || ""))
                score += 18;
            if (/twitter|x/i.test(String(entry.title || "")))
                score += 10;
            return score;
        }, "x_scoped_search", `${note} Browser extraction failed, and a broader Twitter/X-branded query was used only after the scoped fallback returned nothing useful: ${browserError || "unknown browser error"}`);
    };
    const searchGitHubRepositories = async (queryValue, limitValue) => {
        ...
        return {
            ...
            apiUrl,
            results: results.slice(0, limitValue).map((entry) => ({
    ...
    const searchNpmRegistry = async (queryValue, limitValue) => {
    ...
    const searchFetchFirstWorkingMirror = async (label, urls, timeoutForCall = 30000) => {
        const errors = [];
    ...
    const searchAnnasArchive = async (queryValue, limitValue) => {
        const mirrors = [
        ...
        for (const searchUrl of searchUrls) {
            try {
                const directSearch = await runBrowserBackedWebsiteSearch("annas_archive", searchUrl, limitValue, [
                    "      const normalize = (value) => String(value || \"\").replace(/\\s+/g, \" \").trim();",
                    "      const seen = new Set();",
                    "      const results = [];",
    ...
    const searchLibgen = async (queryValue, limitValue) => {
        const mirrors = [
            "https://libgen.li",
            "https://libgen.la",
            "https://libgen.gl",
        ...
        for (const searchUrl of searchUrls) {
            try {
                const directSearch = await runBrowserBackedWebsiteSearch("libgen", searchUrl, limitValue, [
                    "      const normalize = (value) => String(value || \"\").replace(/\\s+/g, \" \").trim();",
                    "      const results = [];",
    ...
    const searchArxiv = async (queryValue, limitValue) => {
    ...
    const searchYouTube = async (queryValue, limitValue) => {
        ...
        if (!initialDataMatch)
            throw new Error("YouTube did not expose ytInitialData on the search results page.");
    ...
    const searchMdnIndex = async (queryValue, limitValue, locale = "en-US") => {
        ...
        const scored = items
            ...
            if (titleLower.includes(normalizedQuery))
                score += 40;
            if (slugTail.includes(normalizedQuery))
                score += 25;
            if (titleLower === normalizedQuery || slugTail === normalizedQuery)
                score += 60;
            return {
                title,
                url: resolvedUrl,
    ...
    const searchMicrosoftLearn = async (queryValue, limitValue, locale = "en-us") => {
    ...
    const searchSlashdot = async (queryValue, limitValue) => {
    ...
    const specificWebsiteFallbackReason = (websiteName) => ({
    ...
    const runSpecificWebsiteSearch = async (websiteName, queryValue, limitValue, languageValue, subredditValue, sortValue, timeValue, siteValue, preferredProvider) => {
        switch (websiteName) {
            case "archive":
    ...
    const configString = (fieldName) => ctl.getPluginConfig(configSchematics).get(fieldName)?.trim() || "";
    const qbtBaseUrl = (value) => (String(value || "").trim() || configString("qbittorrentUrl") || "http://127.0.0.1:8080").replace(/\/+$/, "");
    const qbtUsername = (value) => String(value || "").trim() || configString("qbittorrentUsername");
    const qbtPassword = (value) => String(value || "") || configString("qbittorrentPassword");
    const seerrBaseUrl = (value) => (String(value || "").trim() || configString("seerrUrl") || "http://127.0.0.1:5055").replace(/\/+$/, "");
    const seerrToken = (value) => String(value || "").trim() || configString("seerrApiKey");
    const isLocalQbtBaseUrl = (baseUrl) => {
        const parsed = safeUrl(baseUrl);
        const host = String(parsed?.hostname || "").toLowerCase();
        return host === "localhost" || host === "127.0.0.1" || host === "::1" || host === "[::1]";
    };
    const connectQbt = async (baseUrl, username, password) => {
        requireCommandExecution();
        const api = require("qbittorrent-api-v2");
        return await api.connect(baseUrl, username, password);
    };
    const qbtLoginCookie = async (baseUrl, username, password) => {
        ...
        if (!response.ok)
            throw new Error(`qBittorrent login failed with status ${response.status}: ${response.statusText}`);
        const text = await response.text();
        if (!/Ok\./i.test(text))
            throw new Error("qBittorrent login failed.");
        return response.headers.get("set-cookie") || "";
    };
    const qbtBypassSessionCookie = async (baseUrl) => {
        const response = await fetch(`${baseUrl}/api/v2/app/version`, {
            method: "GET",
            signal: AbortSignal.timeout(30000),
        });
        if (!response.ok)
            throw new Error(`qBittorrent anonymous session bootstrap failed with status ${response.status}: ${response.statusText}`);
        await response.text();
        return response.headers.get("set-cookie") || "";
    };
    const qbtSearchSessionStorePath = path.join(workspaceRoot, ".agentic-studio", "qbittorrent-search-sessions.json");
    const readQbtSearchSessionStore = async () => {
        try {
    ...
    const writeQbtSearchSessionStore = async (store) => {
    ...
    const persistQbtSearchSession = async (baseUrl, searchId, cookie) => {
        if (!cookie || !Number.isFinite(searchId))
            return;
    ...
    const loadQbtSearchSessionCookie = async (baseUrl, searchId) => {
        const store = await readQbtSearchSessionStore();
    ...
    const clearQbtSearchSession = async (baseUrl, searchId) => {
        const store = await readQbtSearchSessionStore();
        delete store[`${baseUrl}::${searchId}`];
        await writeQbtSearchSessionStore(store);
    };
    const qbtApiRequest = async (baseUrl, cookie, endpoint, form) => {
        const headers = {};
        if (cookie)
        ...
        const text = await response.text();
        if (!response.ok)
            throw new Error(`qBittorrent API ${endpoint} failed with status ${response.status}: ${text || response.statusText}`);
        try {
    ...
    const qbtApiGet = async (baseUrl, cookie, endpoint, params) => {
        const searchParams = new URLSearchParams();
    ...
    const qbtApiGetWithFallbacks = async (baseUrl, cookie, endpoint, paramCandidates) => {
        let lastError;
        for (const params of paramCandidates) {
            try {
                ...
                lastError = error;
                if (!/status (400|404)\b/i.test(String(error.message || "")))
                    throw error;
            }
        }
        throw lastError instanceof Error ? lastError : new Error(`qBittorrent API ${endpoint} failed.`);
    };
    const qbtApiPostWithFallbacks = async (baseUrl, cookie, endpoint, formCandidates) => {
        let lastError;
        for (const form of formCandidates) {
            try {
                ...
                lastError = error;
                if (!/status (400|404|405)\b/i.test(String(error.message || "")))
                    throw error;
            }
        }
        throw lastError instanceof Error ? lastError : new Error(`qBittorrent API ${endpoint} failed.`);
    };
    const resolveQbtAuthContext = async (baseUrl, username, password) => {
        ...
        if (username && password) {
            try {
                ...
                return {
                    ...
                    cookie: "",
                    warning: `qBittorrent login failed and fell back to localhost bypass mode: ${error.message}`,
                };
            }
        }
        if (canBypassLocalAuth) {
    ...
    const createQbtHttpClient = async (baseUrl, authContext) => {
        const cookie = String(authContext.cookie || "");
        const postWithFallback = async (primaryEndpoint, fallbackEndpoint, form) => {
            try {
                return await qbtApiRequest(baseUrl, cookie, primaryEndpoint, form);
            }
            catch (error) {
                if (!fallbackEndpoint || !/status 404/i.test(String(error.message || "")))
                    throw error;
        ...
        return {
            ...
            torrents: async (filter, category, sort, reverse, limit, offset, hashes) => await qbtApiGet(baseUrl, cookie, "/torrents/info", { filter, category, sort, reverse, limit, offset, hashes }),
            deleteTorrents: async (hashes, deleteFiles) => await qbtApiRequest(baseUrl, cookie, "/torrents/delete", new URLSearchParams({ hashes, deleteFiles: String(Boolean(deleteFiles)) })),
            pauseTorrents: async (hashes) => await postWithFallback("/torrents/stop", "/torrents/pause", new URLSearchParams({ hashes })),
            resumeTorrents: async (hashes) => await postWithFallback("/torrents/start", "/torrents/resume", new URLSearchParams({ hashes })),
            recheckTorrents: async (hashes) => await qbtApiRequest(baseUrl, cookie, "/torrents/recheck", new URLSearchParams({ hashes })),
            reannounceTorrents: async (hashes) => await qbtApiRequest(baseUrl, cookie, "/torrents/reannounce", new URLSearchParams({ hashes })),
            files: async (hash) => await qbtApiGet(baseUrl, cookie, "/torrents/files", { hash }),
            properties: async (hash) => await qbtApiGet(baseUrl, cookie, "/torrents/properties", { hash }),
            trackers: async (hash) => await qbtApiGet(baseUrl, cookie, "/torrents/trackers", { hash }),
            setLocation: async (hashes, location) => await qbtApiRequest(baseUrl, cookie, "/torrents/setLocation", new URLSearchParams({ hashes, location })),
            setCategory: async (hashes, category) => await qbtApiRequest(baseUrl, cookie, "/torrents/setCategory", new URLSearchParams({ hashes, category })),
            setDownloadLimit: async (hashes, limit) => await qbtApiRequest(baseUrl, cookie, "/torrents/setDownloadLimit", new URLSearchParams({ hashes, limit: String(limit) })),
            setUploadLimit: async (hashes, limit) => await qbtApiRequest(baseUrl, cookie, "/torrents/setUploadLimit", new URLSearchParams({ hashes, limit: String(limit) })),
            setShareLimit: async (hashes, ratioLimit, seedingTimeLimit) => await qbtApiRequest(baseUrl, cookie, "/torrents/setShareLimits", new URLSearchParams({ hashes, ratioLimit: String(ratioLimit), seedingTimeLimit: String(seedingTimeLimit) })),
            setForceStart: async (hashes, value) => await qbtApiRequest(baseUrl, cookie, "/torrents/setForceStart", new URLSearchParams({ hashes, value: String(Boolean(value)) })),
            setSuperSeeding: async (hashes, value) => await qbtApiRequest(baseUrl, cookie, "/torrents/setSuperSeeding", new URLSearchParams({ hashes, value: String(Boolean(value)) })),
            toggleSequentialDownload: async (hashes) => await qbtApiRequest(baseUrl, cookie, "/torrents/toggleSequentialDownload", new URLSearchParams({ hashes })),
            toggleFirstLastPiecePrio: async (hashes) => await qbtApiRequest(baseUrl, cookie, "/torrents/toggleFirstLastPiecePrio", new URLSearchParams({ hashes })),
            searchPlugins: async () => await qbtApiGet(baseUrl, cookie, "/search/plugins"),
            startSearch: async (pattern, plugins, category) => await qbtApiRequest(baseUrl, cookie, "/search/start", new URLSearchParams({ pattern, plugins, category })),
            searchStatus: async (searchId) => searchId === undefined
            ...
            searchResults: async (searchId, limitValue, offsetValue) => await qbtApiGetWithFallbacks(baseUrl, cookie, "/search/results", [
                { searchId, limit: limitValue, offset: offsetValue },
                { id: searchId, limit: limitValue, offset: offsetValue },
            ]),
            deleteSearch: async (searchId) => await qbtApiPostWithFallbacks(baseUrl, cookie, "/search/delete", [
                new URLSearchParams({ searchId: String(searchId) }),
                new URLSearchParams({ id: String(searchId) }),
            ]),
        };
    };
    const connectOrBypassQbt = async (baseUrl, username, password) => {
        ...
        if (String(authContext.authMode) === "login") {
            try {
                ...
                if (!isLocalQbtBaseUrl(baseUrl))
                    throw error;
                const bypassContext = { authMode: "localhost_bypass", cookie: "", warning: `qBittorrent client login failed and fell back to localhost bypass mode: ${error.message}` };
    ...
    const seerrApiRequest = async (baseUrl, apiKey, endpoint, method, body, timeoutForCall) => {
        if (!apiKey)
            throw new Error("Seerr API key is required. Pass seerr_api_key or configure Seerr API Key in plugin settings.");
    ...
    const addDownloadHintsToSearchResults = async (results, maxArchiveLinks = 5) => {
        const enriched = [];
        for (const entry of results) {
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_http_request", async ({ url, method, headers_json, body, detail, timeout_ms }) => {
            ...
            const parsedHeaders = JSON.parse(headers_json);
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_http_wait", async ({ url, method, headers_json, expect_status, contains_text, regex, detail, timeout_ms, poll_interval_ms }) => {
            ...
            const pattern = String(regex || "").trim() ? new RegExp(regex) : null;
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_web_download", async ({ url, output_path, overwrite, force_download, download_index, scan_limit, detail, timeout_ms }) => {
            ...
            if (isCommonYtDlpHost(downloadUrl) && !isProbablyDownloadableUrl(downloadUrl)) {
                return await runVideoDownloadRedirect(downloadUrl, output_path, overwrite, timeout_ms);
            }
            if (/\/\/(?:drive|docs)\.google\.com\//i.test(downloadUrl)) {
                return json(await fetchDownloadToFile(downloadUrl, output_path, overwrite, timeout_ms, transformed.note));
            }
            ...
            if (archiveIdentifier && !parsed?.pathname.startsWith("/download/")) {
                const downloadLinks = await archiveDownloadLinks(decodeURIComponent(archiveIdentifier), scan_limit);
                ...
                if (chosen) {
                    return json(await fetchDownloadToFile(String(chosen.url), output_path, overwrite, timeout_ms, transformed.note));
                }
                return json({
                    success: false,
                    detail: detailLevel,
            ...
            const response = await fetch(downloadUrl, {
                redirect: "follow",
                headers: { "User-Agent": "agentic-studio/1.0" },
                signal: AbortSignal.timeout(Math.min(timeout_ms, 120000)),
            });
            if (!response.ok)
                throw new Error(`Download discovery failed with status ${response.status}: ${response.statusText}`);
            const contentType = response.headers.get("content-type") || "";
            if (!contentTypeLooksHtml(contentType)) {
                return json(await fetchDownloadToFile(response.url || downloadUrl, output_path, overwrite, timeout_ms, transformed.note));
            }
            const html = await response.text();
            const discovered = await discoverDownloadOptions(response.url || downloadUrl, html, scan_limit);
            ...
            return json({
                ...
                downloadOptions: (detailLevel === "compact" ? discovered.slice(0, 10) : discovered).map((entry) => ({
                    ...entry,
                    asWebDownloadArgs: {
                        url: entry.url,
                        output_path,
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_web_extract", async ({ action, url, query, prompt, code, script_js, input_json, output_path, output_directory, format, formats_json, schema_json, extraction_prompt, options_json, detail, wait_until, limit, timeout_ms, poll_interval_ms, wait_timeout_ms }) => {
            ...
            if (selectedAction === "visit") {
                const response = await fetch(url, { redirect: "follow", signal: AbortSignal.timeout(Math.min(timeout_ms, 300000)) });
                const html = await response.text();
                const titleMatch = html.match(/<title[^>]*>([\s\S]*?)<\/title>/i);
                const text = stripHtmlToText(html).slice(0, maxOutputBytes);
                if (detailLevel === "max")
                    return json({ url, finalUrl: response.url, title: stripHtmlToText(titleMatch?.[1] || ""), text });
            ...
            if (selectedAction === "images") {
                const images = await extractPageImages(url, limit);
                if (detailLevel === "max")
                    return json({ url, images });
            ...
            if (selectedAction === "archive") {
                ...
                const result = await executeConfiguredBrowserNodeScript(script, Math.max(timeoutMs, timeout_ms), preferredBrowserBackend);
                return buildCommandResponse(`node archive page ${url}`, result);
            }
            if (selectedAction === "firecrawl_scrape") {
                const formats = JSON.parse(formats_json);
                if (!Array.isArray(formats))
                    throw new Error("formats_json must be a JSON array.");
                const options = parseJsonObject(options_json, "options_json");
                const schema = String(schema_json || "").trim() ? parseJsonObject(schema_json, "schema_json") : null;
                const body = mergeDefined(options, {
                    url,
                    formats: (() => {
                        const current = [...formats];
                        if (schema || String(extraction_prompt || "").trim()) {
                            current.push(mergeDefined({ type: "json", prompt: String(extraction_prompt || "").trim() || "Extract the requested structured data from the page." }, schema ? { schema } : {}));
                        }
                        return current;
                    })(),
                });
                const response = await firecrawlApiRequest(ctl, "/v2/scrape", "POST", body, timeout_ms);
                if (String(output_path || "").trim()) {
                    const targetPath = resolveInsideWorkspace(workspaceRoot, output_path);
            ...
            if (selectedAction === "firecrawl_map") {
                const response = await firecrawlApiRequest(ctl, "/v2/map", "POST", mergeDefined(parseJsonObject(options_json, "options_json"), { url, search: String(query || "").trim() || undefined }), timeout_ms);
                if (detailLevel === "max")
                    return json(response);
            ...
            if (selectedAction === "firecrawl_search") {
                const response = await firecrawlApiRequest(ctl, "/v2/search", "POST", mergeDefined(parseJsonObject(options_json, "options_json"), { query }), timeout_ms);
                if (detailLevel === "max")
                    return json(response);
            ...
            if (selectedAction === "firecrawl_crawl") {
                const started = await firecrawlApiRequest(ctl, "/v2/crawl", "POST", mergeDefined(parseJsonObject(options_json, "options_json"), { url }), Math.min(wait_timeout_ms, 120000));
                const crawlId = String(started.id || started.jobId || started.data?.id || "");
                if (!crawlId)
                    throw new Error("Firecrawl crawl did not return a job id.");
                const completed = await firecrawlPollUntilDone(ctl, `/v2/crawl/${crawlId}`, poll_interval_ms, wait_timeout_ms);
            ...
            if (selectedAction === "firecrawl_agent") {
                const schema = String(schema_json || "").trim() ? parseJsonObject(schema_json, "schema_json") : undefined;
                const started = await firecrawlApiRequest(ctl, "/v2/agent", "POST", mergeDefined({ prompt, urls: [url], schema, maxCredits: 2500, model: "spark-1-mini" }), Math.min(wait_timeout_ms, 120000));
                const jobId = String(started.id || started.jobId || started.data?.id || "");
                if (!jobId)
                    throw new Error("Firecrawl agent did not return a job id.");
                const completed = await firecrawlPollUntilDone(ctl, `/v2/agent/${jobId}`, poll_interval_ms, wait_timeout_ms);
            ...
            if (selectedAction === "firecrawl_interact") {
                if (!String(prompt || "").trim() && !String(code || "").trim())
                    throw new Error("Provide prompt or code.");
                const scrapeResponse = await firecrawlApiRequest(ctl, "/v2/scrape", "POST", { url, formats: JSON.parse(formats_json) }, Math.min(timeout_ms, 120000));
                const scrapeData = scrapeResponse.data || {};
                const scrapeMeta = scrapeData.metadata || {};
                ...
                if (!scrapeId)
                    throw new Error("Firecrawl scrape did not return a scrapeId for interact.");
                const interactResponse = await firecrawlApiRequest(ctl, `/v2/scrape/${scrapeId}/interact`, "POST", mergeDefined({ prompt: String(prompt || "").trim() || undefined, code: String(code || "").trim() || undefined, language: "node", timeout: Math.ceil(Number(timeout_ms) / 1000) }), timeout_ms);
            ...
            if (selectedAction === "browser_script") {
                ...
                const preferredBrowserBackend = getBrowserAutomationBackend();
                const outputDirectory = resolveInsideWorkspace(workspaceRoot, output_directory);
                ...
                const synthesizeBrowserScriptFromPrompt = (promptText) => {
                ...
                const script = [
                    ...
                    `const input = JSON.parse(${JSON.stringify(input_json)});`,
                ...
                if (browserResult && typeof browserResult === "object") {
                    ...
                    const preview = Object.fromEntries(Object.entries(browserResult).slice(0, 12));
                    const serialized = JSON.stringify(browserResult, null, 2);
                    const compactPayload = {
                        ...responseMeta,
                        resultPreview: preview,
                        resultKeys: Object.keys(browserResult).slice(0, 30),
                        resultLength: serialized.length,
                    };
                    if (serialized.length > TEXT_PREVIEW_CHARS) {
                        compactPayload.resultReportPath = await maybeWriteToolOutputToFile(workspaceRoot, createReportPath("as_web_extract", "browser-script-result"), browserResult);
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_multi_website_search", async ({ website, query, limit, language, subreddit, sort, time, site, search_engine, detail }) => {
            ...
            try {
                const specificSearch = await runSpecificWebsiteSearch(selectedWebsite, query, limit, language, subreddit, sort, time, site, search_engine);
                if (specificSearch) {
                    if (detailLevel === "max")
                        return json({ query, website: selectedWebsite, ...specificSearch });
                ...
                if (fallbackTargetSite) {
                    const scopedQuery = buildScopedQuery(query, fallbackTargetSite);
                    const fallback = await runSearchProvider(scopedQuery, limit, search_engine, { language });
                    ...
                    if (detailLevel === "max") {
                        return json({
                            ...
                            unsupportedReason,
                            warning: `Site-specific search failed and fell back to a scoped web search: ${error.message}`,
                            results: resultList,
                        });
                    }
                    return json(await compactCollectionPayload("as_multi_website_search", detailLevel, {
            ...
            const scopedQuery = buildScopedQuery(query, targetSite);
            const { provider, results, mirror } = await runSearchProvider(scopedQuery, limit, search_engine, { language });
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_torrent_controller", async ({ action, qbittorrent_url, qbittorrent_username, qbittorrent_password, source, torrent_file, save_path, category, tags, hashes, delete_files, paused, sequential_download, first_last_piece_priority, filter, sort, reverse, limit, offset, options_json, search_query, search_plugins, search_category, search_id, seerr_url, seerr_api_key, seerr_endpoint, seerr_method, media_type, media_id, detail, timeout_ms, }) => {
            const selectedAction = String(action);
            const detailLevel = normalizeDetailLevel(detail);
            const opts = parseJsonObject(options_json, "options_json");
            const hasSearchId = search_id !== undefined && search_id !== null && String(search_id).trim() !== "" && Number.isFinite(Number(search_id));
            const compactJsonPayload = async (backend, payload) => {
                if (detailLevel === "max")
                    return json(payload);
                ...
                if (payload && typeof payload === "object") {
                    ...
                    if (Array.isArray(objectPayload.files)) {
                        const files = objectPayload.files.map((entry) => compactTorrentFileRecord(entry));
                    ...
                    for (const key of ["results", "torrents", "files", "trackers", "plugins", "status", "search"]) {
                        if (Array.isArray(objectPayload[key])) {
                            return json(await compactCollectionPayload("as_torrent_controller", detailLevel, { action: selectedAction, backend }, key, objectPayload[key], compactTorrentRecord));
                        }
                    }
                    if (detailLevel === "full")
                        return json(objectPayload);
            ...
            if (selectedAction.startsWith("seerr_")) {
                const baseUrl = seerrBaseUrl(seerr_url);
                const apiKey = seerrToken(seerr_api_key);
                if (selectedAction === "seerr_status") {
                    return await compactJsonPayload("seerr", await seerrApiRequest(baseUrl, apiKey, "/api/v1/status", "GET", null, timeout_ms));
                }
                if (selectedAction === "seerr_search") {
                    if (!String(search_query || "").trim())
                        throw new Error("search_query is required for seerr_search.");
                    return await compactJsonPayload("seerr", await seerrApiRequest(baseUrl, apiKey, `/api/v1/search?query=${encodeURIComponent(search_query)}`, "GET", null, timeout_ms));
                }
                if (selectedAction === "seerr_requests") {
                    return await compactJsonPayload("seerr", await seerrApiRequest(baseUrl, apiKey, `/api/v1/request?take=${Number(limit)}&skip=${Number(offset)}`, "GET", null, timeout_ms));
                }
                if (selectedAction === "seerr_request") {
                    if (!Number(media_id))
                        throw new Error("media_id is required for seerr_request.");
                ...
                if (selectedAction === "seerr_raw") {
                    if (!String(seerr_endpoint || "").trim())
                        throw new Error("seerr_endpoint is required for seerr_raw.");
                    return await compactJsonPayload("seerr", await seerrApiRequest(baseUrl, apiKey, seerr_endpoint, seerr_method, opts, timeout_ms));
                }
            }
            const baseUrl = qbtBaseUrl(qbittorrent_url);
            const username = qbtUsername(qbittorrent_username);
            const password = qbtPassword(qbittorrent_password);
            if (selectedAction === "add") {
                const authContext = await resolveQbtAuthContext(baseUrl, username, password);
                const cookie = String(authContext.cookie || "");
                const form = new FormData();
                if (String(torrent_file || "").trim()) {
                    const torrentPath = resolveInsideWorkspace(workspaceRoot, torrent_file);
                ...
                if (String(save_path || "").trim())
                    form.append("savepath", String(save_path));
            ...
            if (selectedAction === "list") {
                return await compactJsonPayload("qbittorrent", mergeDefined({ backend: "qbittorrent", authMode: authContext.authMode, warning: authContext.warning, torrents: await qbt.torrents(filter || undefined, category || undefined, sort || undefined, reverse, limit, offset, selectedHashes || undefined) }));
            }
            if (selectedAction === "remove") {
                await qbt.deleteTorrents(selectedHashes, delete_files);
            ...
            if (selectedAction === "files")
                return await compactJsonPayload("qbittorrent", mergeDefined({ backend: "qbittorrent", authMode: authContext.authMode, warning: authContext.warning, hashes: selectedHashes, files: await qbt.files(selectedHashes) }));
            if (selectedAction === "properties")
                return await compactJsonPayload("qbittorrent", mergeDefined({ backend: "qbittorrent", authMode: authContext.authMode, warning: authContext.warning, hashes: selectedHashes, properties: await qbt.properties(selectedHashes) }));
            if (selectedAction === "trackers")
                return await compactJsonPayload("qbittorrent", mergeDefined({ backend: "qbittorrent", authMode: authContext.authMode, warning: authContext.warning, hashes: selectedHashes, trackers: await qbt.trackers(selectedHashes) }));
            ...
            if (selectedAction === "search_plugins")
                return await compactJsonPayload("qbittorrent", mergeDefined({ backend: "qbittorrent", authMode: authContext.authMode, warning: authContext.warning, plugins: await qbt.searchPlugins() }));
            if (selectedAction === "search_start") {
                if (!String(search_query || "").trim())
                    throw new Error("search_query is required for search_start.");
                const search = await qbt.startSearch(search_query, search_plugins, search_category);
                const startedSearchId = Number(search?.id);
            ...
            if (selectedAction === "search_results") {
                if (!hasSearchId)
                    throw new Error("search_id is required for search_results.");
            ...
            if (selectedAction === "search_delete") {
                if (!hasSearchId)
                    throw new Error("search_id is required for search_delete.");
                const sessionCookie = await loadQbtSearchSessionCookie(baseUrl, Number(search_id));
                const sessionClient = sessionCookie
                    ? await createQbtHttpClient(baseUrl, mergeDefined(authContext, { cookie: sessionCookie }))
                    : qbt;
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_deno_run_script", async (params) => {
            ...
            const denoExecutable = await getDenoExecutablePath(ctl);
            const cwd = resolveInsideWorkspace(workspaceRoot, working_directory);
            ...
            const runScriptFile = async (scriptFilePath, label) => {
                ...
                const result = await executeManagedCommand(ctl, command, { cwd, shell, env }, Math.max(timeoutMs, timeout_ms), maxOutputBytes);
                return {
                    script: label,
                    ...buildCommandResponsePayload(command, result),
                };
            ...
            try {
                ...
                await fsp.writeFile(tempScriptPath, script_ts, "utf8");
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_web_search", async ({ query, search_engine, site, limit, searxng_list_mirrors, searxng_mirror_index, searxng_mirror_max, searxng_mirror_url, detail }) => {
            ...
            if (searxng_list_mirrors) {
                const mirrorList = await listSearxMirrors(searxng_mirror_index, searxng_mirror_max);
                return json(mirrorList);
            }
            if (!String(query || "").trim()) {
                throw new Error("query is required unless searxng_list_mirrors=true.");
            }
            const scopedQuery = buildScopedQuery(query, site);
            const searchResult = await runSearchProvider(scopedQuery, limit, search_engine, {
                language: "all",
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_web_image_search", async ({ query, limit, detail }) => {
            const detailLevel = normalizeDetailLevel(detail);
            const results = await fallbackImageSearch(query, limit);
            if (detailLevel === "max")
                return json({ query, provider: "bing_images_html", results });
