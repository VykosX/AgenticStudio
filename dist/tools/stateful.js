...
exports.registerStatefulTools = registerStatefulTools;
const providerSkills_1 = require("../shared/providerSkills");
function registerStatefulTools(ctx, tools) {
    const { tool, z, safeTool, workspaceRoot, pluginDataDirectory, resolveInsideDirectory, readJsonFile, writeJsonFile, readMergedRecords, resolveMemoryPaths, resolveTodoPaths, fileExists, skillsDirectory, dynamicToolNameSchema, mergeDefined, path, fsp, json, normalize, ctl, configSchematics } = ctx;
    const agentTaskStatuses = ["open", "in_progress", "blocked", "done", "cancelled"];
    const agentTaskPriorities = ["low", "normal", "high", "critical"];
    const scopeLabel = (global, context) => global ? "global" : context.mode;
    const currentSkillQueryOptions = () => ({
        includePerAppAutomationSkills: (ctl.getPluginConfig(configSchematics).get("enablePerAppAutomationGuidance") ?? true) === true,
    });
    ...
    const readAgentTasks = async () => readJsonFile(await agentTasksPath(), []);
    const writeAgentTasks = async (items) => {
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_memory_list", async ({ query, tag, global, limit }) => {
            ...
            if (tag.trim()) {
                ...
                entries = entries.filter((entry) => Array.isArray(entry.tags) && entry.tags.some((item) => normalize(String(item)) === wanted));
            }
            return json({ scope: scopeLabel(Boolean(global), context), count: entries.length, entries: entries.slice(0, limit) });
        }),
    }));
    tools.push(tool({
        ...
        implementation: safeTool("as_agent_task", async ({ action, id, title, description, status, priority, assignee, tags, notes, due_at, parent_id, query, limit, patch_json, text }) => {
            ...
            if (selectedAction === "create") {
                if (!String(title || "").trim())
                    throw new Error("title is required for action=create.");
                const crypto = await Promise.resolve().then(() => __importStar(require("crypto")));
            ...
            if (selectedAction === "list") {
                ...
                if (String(assignee || "").trim()) {
                    const wanted = normalize(assignee);
                ...
                return json({ count: items.length, tasks: items.slice(0, limit) });
            }
            if (selectedAction === "next") {
                ...
                tasks.sort((left, right) => (priorityWeight[String(right.priority || "normal")] || 0) - (priorityWeight[String(left.priority || "normal")] || 0)
            ...
            if (selectedAction === "update") {
                ...
                if (!patch || typeof patch !== "object" || Array.isArray(patch))
                    throw new Error("patch_json must be a JSON object.");
                const items = await readAgentTasks();
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_skill_recommend", async ({ query, category, limit, include_builtin, include_custom }) => {
            const wanted = normalize(query);
            const categoryWanted = normalize(category);
            const words = wanted.split(/\s+/).filter(Boolean);
            const hasAny = (...terms) => terms.some((term) => wanted.includes(normalize(term)));
            ...
            const skills = (await (0, providerSkills_1.listAllSkills)(currentSkillQueryOptions()))
            ...
            const scored = skills.map((skill) => {
                ...
                if (categoryWanted && normalize(String(skill.category || "")) === categoryWanted)
                    score += 3;
                if (normalizedId === "desktop/per-app-automation-guidance") {
                    if (perAppAutomationGuidanceEnabled && desktopIntent) {
                        score += 12;
                        reasons.push("single-app desktop workflow");
                    }
                    if (singleAppIntent) {
                        score += 14;
                        reasons.push("app-focused interaction guidance");
                    }
                    if (supportedPerAppIntent) {
                        score += 6;
                        reasons.push("supported app notes");
                    }
                    if (hasAny("msi afterburner", "afterburner", "fan speed", "slider")) {
                        score += 8;
                        reasons.push("custom-control app notes");
                    }
                    if (visionIntent)
                        score += 3;
                    if (specificLocalBrowserIntent)
                        score -= 18;
                    if (loopIntent)
                        score -= 3;
                    if (formIntent)
                    ...
                    if (desktopIntent) {
                        score += 10;
                        reasons.push("specific desktop interaction workflow");
                    }
                    if (visionIntent) {
                        score += 8;
                        reasons.push("vision-first screenshot verification");
                    }
                    if (hasAny("calculator", "menu", "button", "window", "click", "drag", "cursor", "afterburner", "slider", "fan speed"))
                        score += 6;
                    if (specificLocalBrowserIntent) {
                        score += 14;
                        reasons.push("installed browser window automation");
                    }
                    if (autonomousBrowserIntent && !specificLocalBrowserIntent)
                        score -= 10;
                    if (perAppAutomationGuidanceEnabled && singleAppIntent)
                        score -= 6;
                    if (formIntent)
                        score -= 2;
                }
                    ...
                    if (browserIntent) {
                        score += 12;
                        reasons.push("browser-driven webpage workflow");
                    }
                    if (autonomousBrowserIntent) {
                        score += 18;
                        reasons.push("autonomous browser navigation");
                    }
                    if (hasAny("site search", "search page", "follow links", "link graph", "quora", "twitter", "x", "youtube", "reddit", "github", "browser script", "headless browser"))
                        score += 8;
                    if (specificLocalBrowserIntent)
                        score -= 16;
                    if (desktopIntent && !browserIntent)
                        score -= 6;
                    if (formIntent)
                        score -= 2;
                }
                    ...
                    if (formIntent) {
                        score += 16;
                        reasons.push("form/data-entry workflow");
                    }
                    if (hasAny("csv", "record", "records", "field", "fields", "submit"))
                        score += 8;
                    if (hasAny("calculator", "menu", "hamburger"))
                        score -= 8;
                }
                else if (normalizedId === "automation/safe-desktop-agent-loop") {
                    if (desktopIntent && loopIntent) {
                        score += 14;
                        reasons.push("recoverable desktop observe-act loop");
                    }
                    else if (desktopIntent && automationIntent)
                        score += 4;
                    if (!desktopIntent)
                        score -= 2;
                }
                    ...
                    if (desktopIntent) {
                        score += 7;
                        reasons.push("direct local app operation");
                    }
                    if (visionIntent)
                        score -= 2;
                    if (loopIntent)
                    ...
                    if (visionIntent) {
                        score += 10;
                        reasons.push("image/screenshot inspection");
                    }
                    if (!desktopIntent)
                        score += 4;
                    if (desktopIntent && hasAny("click", "type", "window", "drag", "button"))
                        score -= 8;
                }
                else if (normalizedId === "automation/durable-agent-work") {
                    if (loopIntent && hasAny("queue", "task registry", "ownership", "parallel", "compaction", "restart")) {
                        score += 12;
                        reasons.push("durable orchestration and recovery");
                    }
                    if (desktopIntent && !loopIntent)
                        score -= 8;
                }
                    ...
                    if (loopIntent) {
                        score += 8;
                        reasons.push("generic plan-do-verify loop");
                    }
                    if (desktopIntent)
                        score -= 5;
                }
    ...
    tools.push(tool({
        ...
        implementation: safeTool("as_skill", async ({ action, name, content, summary, overwrite, query, category, limit }) => {
            const selectedAction = String(action);
            if (selectedAction === "list") {
                let skills = await (0, providerSkills_1.listAllSkills)(currentSkillQueryOptions());
                ...
                return json({ count: skills.length, skills: skills.slice(0, limit) });
            }
            if (selectedAction === "read") {
                const skill = await (0, providerSkills_1.resolveSkillRecord)(name, currentSkillQueryOptions());
                if (!skill)
                    throw new Error(`Skill '${name}' was not found.`);
                const skillContent = await (0, providerSkills_1.readSkillContent)(skill);
                return json({
                    name: skill.id,
                    source: skill.source,
                    category: skill.category,
            ...
            if (selectedAction === "create") {
                ...
                if (!overwrite && await fileExists(fullPath))
                    throw new Error(`Skill already exists: ${safeName}`);
            ...
            if (selectedAction === "delete") {
                const fullPath = resolveInsideDirectory(skillsDirectory(workspaceRoot), `${name}.md`);
